...
function [ANLZ,err] = Relax_T2NaBiexDistChng_v1a_Func(ANLZ,INPUT)
...
[beta,resid,jacob,sigma,mse] = nlinfit(Del,Vals,@Reg,Est,options);
...
hold on
t = (0:0.1:60);
plot(t,beta(1)*(beta(4)*exp(-t/beta(2))+(1-beta(4))*exp(-t/beta(3))));
...
Panel(5,:) = {'%Fast',[num2str(beta(4),'%2.3g'),' +/- ',num2str(ci(4),'%2.2g')],'Output'};
Panel(6,:) = {'Rsqr',Rsqr,'Output'};
ANLZ.Panel = Panel;
ANLZ.PanelOutput = cell2struct(Panel,{'label','value','type'},2);

ANLZ.Name = 'T2NaBiexDistChng';
...
%=======================================================
function F = Reg(P,t) 
F = P(1)*(P(4)*exp(-t/P(2))+(1-P(4))*exp(-t/P(3)));
