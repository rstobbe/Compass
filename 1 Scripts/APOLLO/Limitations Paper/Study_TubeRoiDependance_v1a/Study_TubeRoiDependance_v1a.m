...
function [SCRPTipt,SCRPTGBL,err] = Study_TubeRoiDependance_v1a(SCRPTipt,SCRPTGBL)
...
if not(isfield(SCRPTGBL,'SEF_File_Data'))
    if isfield(SCRPTGBL.CurrentTree.('SEF_File').Struct,'selectedfile')
    file = SCRPTGBL.CurrentTree.('SEF_File').Struct.selectedfile;
        if not(exist(file,'file'))
            err.flag = 1;
            err.msg = '(Re) Load SEF_File - path no longer valid';
            ErrDisp(err);
            return
        else
            Status('busy','Load SEF_File');
            load(file);
            saveData.path = file;
            SCRPTGBL.('SEF_File_Data') = saveData;
        end
    else
        err.flag = 1;
        err.msg = '(Re) Load SEF_File';
        ErrDisp(err);
        return
    end
end
if not(isfield(SCRPTGBL,'NEI_File_Data'))
    if isfield(SCRPTGBL.CurrentTree.('NEI_File').Struct,'selectedfile')
    file = SCRPTGBL.CurrentTree.('NEI_File').Struct.selectedfile;
        if not(exist(file,'file'))
            err.flag = 1;
            err.msg = '(Re) Load NEI_File - path no longer valid';
            ErrDisp(err);
            return
        else
            Status('busy','Load NEI_File');
            load(file);
            saveData.path = file;
            SCRPTGBL.('NEI_File_Data') = saveData;
        end
    else
        err.flag = 1;
        err.msg = '(Re) Load NEI_File';
        ErrDisp(err);
        return
    end
end
...
SEF = SCRPTGBL.SEF_File_Data;
NEI = SCRPTGBL.NEI_File_Data;
