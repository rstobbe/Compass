...
function [SCRPTipt,SCRPTGBL,err] = LoadRoisFromFile_v1a_NumFileSel(SCRPTipt,SCRPTGBL)

Status2('busy','Multiple Roi Load',1);
...
answer = inputdlg('How Many Files?','Multiple Roi Load');
if isempty(answer)
    err.flag = 4;
    err.msg = 'Return';
    return
...
if isfield(SCRPTGBL,[callingfuncs{1},'_Data'])
    ...
    for n = 1:MULTIFILELOAD.numfiles
        if isfield(DataArr,['Roi',num2str(n),'_Data'])
            Data = DataArr.(['Roi',num2str(n),'_Data']);
            ...
            SCRPTipt(SCRPTGBL.RWSUI.curpanipt+n).entrystruct.('LoadRoiCur').curloc = Data.path;   
            DataArr2.(['Roi',num2str(n),'_Data']) = DataArr.(['Roi',num2str(n),'_Data']);
        end
    end
