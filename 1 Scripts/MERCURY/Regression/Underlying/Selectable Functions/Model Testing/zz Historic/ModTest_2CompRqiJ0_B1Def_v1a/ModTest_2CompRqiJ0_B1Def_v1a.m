...
function [SCRPTipt,MODTST,err] = ModTest_2CompRqiJ0_B1Def_v1a(SCRPTipt,MODTSTipt)
...
if not(isfield(MODTSTipt,[CallingLabel,'_Data']))
    if isfield(MODTSTipt.('B1File_Excel').Struct,'selectedfile')
        file = MODTSTipt.('B1File_Excel').Struct.selectedfile;
        if not(exist(file,'file'))
            err.flag = 1;
            err.msg = '(Re) Load B1File_Excel';
            ErrDisp(err);
            return
        else
            saveData.path = file;
            MODTSTipt.([CallingLabel,'_Data']).('B1File_Excel_Data') = saveData;
        end
    else
        err.flag = 1;
        err.msg = '(Re) Load B1File_Excel';
...
% Return Panel Input
%---------------------------------------------
MODTST.B1File = MODTSTipt.([CallingLabel,'_Data']).('B1File_Excel_Data');
