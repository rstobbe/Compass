...
///  \\\ to do\\\
///     * make sure best/fastest CUDA routines being used
///     * zerofill input data to multiple of chunksize (outside)
///         so (j < DatLen) check not necessary  
///==========================================================

extern "C" void ConvSamp2GridSplitRDM(int Count, size_t *HSampDat, size_t *HGrdDat, size_t *HKx, size_t *HKy, size_t *HKz, size_t *HKern, 
    ...
    for(c=-chW; c<=chW+1; c++) {
        for(b=-chW; b<=chW+1; b++) {
            for(a=-chW; a<=chW+1; a++) {
                ...
                // atomicAdd((dGDat+(xflr+a-1)+((yflr+b-1)*GrdDatSz)+((zflr+c-1)*GrdDatSz*GrdDatSz)),(KernVal*DatVal));  CUDA version
				}
			}
		}  
	}
...
/// Code Entry
///=====================================================
void ConvSamp2GridSplitRDM(int Count, size_t *HSampDat, size_t *HGrdDat, size_t *HKx, size_t *HKy, size_t *HKz, size_t *HKern, 
							int GrdDatSz, int DatLen, int KernSz, int iKern, int chW, int SampDatAdr, char* Error){
