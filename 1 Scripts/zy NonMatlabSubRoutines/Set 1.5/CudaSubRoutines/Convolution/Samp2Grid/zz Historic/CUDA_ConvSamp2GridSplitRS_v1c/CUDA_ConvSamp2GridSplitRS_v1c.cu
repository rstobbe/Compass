...
extern "C" void ConvSamp2GridSplitRS(size_t *HSampDat, size_t *HGrdDat, size_t *HKx, size_t *HKy, size_t *HKz, size_t *HKern, 
...
if (j < DatLen) {
    ...
    for(c=-chW; c<=chW+1; c++) {
        for(b=-chW; b<=chW+1; b++) {
            for(a=-chW; a<=chW+1; a++) {
				KernVal = dKern[lrintf(fabsf(xF-(a*iKern))) + lrintf(fabsf(yF-(b*iKern)))*KernSz + lrintf(fabsf(zF-(c*iKern)))*KernSz*KernSz];		// individual conversion necessary (i.e. for 'z') because number will be too big for float to handle                KernVal = dKern[lrintf(fabsf(xF-(a*iKern))+(fabsf(yF-(b*iKern))*KernSz)+(fabsf(zF-(c*iKern))*KernSz*KernSz))];
...
/// Code Entry
///=====================================================
void ConvSamp2GridSplitRS(size_t *HSampDat, size_t *HGrdDat, size_t *HKx, size_t *HKy, size_t *HKz, size_t *HKern, 
							int GrdDatSz, int DatLen, int KernSz, int iKern, int chW, int SampDatAdr, char* Error){
