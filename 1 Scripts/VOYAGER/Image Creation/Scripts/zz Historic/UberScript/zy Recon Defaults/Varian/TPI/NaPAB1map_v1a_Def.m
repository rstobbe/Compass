...
function [err] = NaPAB1map_v1a_Def(IMG,FIDpath,RWSUI,CellArray)
...
file.ic = ['IC_NaB1Map_',projset,'.mat'];
path.ic = [SCRPTPATHS.(RWSUI.tab)(RWSUI.panelnum).defrootloc,'\VOYAGER\Select\',USER,'\Image Creation\B1Map\'];
panelnum.ic = 1;
scrptnum.ic = 1;
err = ExtLoadScrptDefault(RWSUI.tab,panelnum.ic,scrptnum.ic,file.ic,path.ic);
if err.flag
    ErrDisp(err);
    return
end
...
%---------------------------------------------
file.prc = 'PRC_NaTPI_B1Map.mat';
path.prc = [SCRPTPATHS(5).defrootloc,'\VOYAGER\Select\',USER,'\Image Processing\Mapping\'];
panelnum.prc = 2;
scrptnum.prc = 1;
