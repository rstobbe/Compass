...
function [SCRPTipt,SCRPTGBL,err] = PopCreate_v1a(SCRPTipt,SCRPTGBL)
...
% Tests
%---------------------------------------------
if not(isfield(SCRPTGBL,'SysFolder_Data'))
    if isfield(SCRPTGBL.CurrentTree.('SysFolder').Struct,'selectedfile')
        file = SCRPTGBL.CurrentTree.('SysFolder').Struct.selectedfile;
        if not(exist(file,'file'))
            err.flag = 1;
            err.msg = '(Re) Load SysFolder';
            ErrDisp(err);
            return
        else
            saveData.path = file;
            SCRPTGBL.('SysFolder_Data') = saveData;
        end
    else
        err.flag = 1;
        err.msg = '(Re) Load SysFolder';
...
IMG.SysFolder = SCRPTGBL.('SysFolder_Data').path;
IMG.localfolder = SCRPTGBL.('LocalFolder_Data').path;
IMG.function = SCRPTGBL.CurrentTree.('Function');
