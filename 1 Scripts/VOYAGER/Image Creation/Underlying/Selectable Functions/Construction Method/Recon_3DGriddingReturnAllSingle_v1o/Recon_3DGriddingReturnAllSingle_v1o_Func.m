...
function [RECON,err] = Recon_3DGriddingReturnAllSingle_v1o_Func(RECON,INPUT)
    ...
    for m = 1:Nexp
        for q = 1:Naverages
            for r = 1:Nechos
                ...
                if p == 1
                    sz = size(Im0);
                    ImArr = complex(zeros([sz Nexp Nrcvrs Naverages Nechos],'single'),zeros([sz Nexp Nrcvrs Naverages Nechos],'single'));
                end
                ImArr(:,:,:,m,n,q,r) = Im0;
                p = p+1;
            end
