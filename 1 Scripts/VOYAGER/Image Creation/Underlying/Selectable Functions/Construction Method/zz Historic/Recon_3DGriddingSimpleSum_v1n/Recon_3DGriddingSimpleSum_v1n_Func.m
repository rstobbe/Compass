...
function [RECON,err] = Recon_3DGriddingSimpleSum_v1n_Func(RECON,INPUT)
        ...
        for q = 1:Naverages
            for r = 1:Nechos
                ...
                if strcmp(RECON.visuals,'SingleIm') || strcmp(RECON.visuals,'NewSingleIm') || strcmp(RECON.visuals,'MultiIm')
                    ...
                    maxval = max(max(max(abs(ImArr(sz/8:7*sz/8,sz/8:7*sz/8,sz/8:7*sz/8,:,:,:)))))*1.1;
                    subplot(2,3,4);
                    ImAx = squeeze(abs(permute(ImArr(:,:,sz(2)/2,:,:,:),[2,1,3])));
                    ImAx = flip(ImAx,2);
                    imshow(ImAx,[0 maxval]);
                    subplot(2,3,5);
                    ImCor = flip(squeeze(abs(permute(ImArr(:,sz(2)/2,:,:,:,:),[3,1,2]))),1);
                    ImCor = flip(ImCor,2);
                    imshow(ImCor,[0 maxval]);
                    ImSag = flip(squeeze(abs(permute(ImArr(sz(2)/2,:,:,:,:,:),[3,2,1]))),1);
