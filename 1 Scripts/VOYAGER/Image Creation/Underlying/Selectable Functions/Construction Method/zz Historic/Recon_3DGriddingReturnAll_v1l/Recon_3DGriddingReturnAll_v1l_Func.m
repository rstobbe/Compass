...
function [RECON,err] = Recon_3DGriddingReturnAll_v1l_Func(RECON,INPUT)
...
%             figure(12341234);
%             sz = size(GrdDat);
%             imshow(real(GrdDat(:,:,round(sz/2))),[0 max(abs(GrdDat(:)))]);
            ...
            if p == 1
                sz = size(Im0);
                ImArr = zeros([sz Nexp Nrcvrs Naverages]);
            end
            ImArr(:,:,:,m,n,q) = Im0;
            p = p+1;
        end
