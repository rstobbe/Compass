...
function [RECON,err] = Recon3DGriddingReturnAll_v1j_Func(RECON,INPUT)
...
for n = 1:Nrcvrs
    for m = 1:Nexp
        %---------------------------------------------
        % Sort Projection Undersampling
        %---------------------------------------------        
        if length(IMP.projsampscnr) ~= IMP.PROJimp.nproj  
            DatMat = zeros(IMP.PROJimp.nproj,IMP.PROJimp.npro);
            DatMat(IMP.projsampscnr,:) = DatArr2Mat(Dat0(:,:,m,n),length(IMP.projsampscnr),IMP.PROJimp.npro);
        else
            DatMat = DatArr2Mat(Dat0(:,:,m,n),IMP.PROJimp.nproj,IMP.PROJimp.npro); 
        end
        ...
        % Plot
        %---------------------------------------------    
        if strcmp(RECON.visuals,'SingleIm') || strcmp(RECON.visuals,'MultiIm')
