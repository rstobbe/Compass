...
function [RECON,err] = Recon_2ImKsCombSuper_v2n_Func(RECON,INPUT)

Status2('busy','Create Image Via Gridding (combine 2 echos)',2);
Status2('done','',3);
...
if Nechos ~= 2
    err.flag = 1;
    err.msg = 'Data must contain 2 echos for ''Recon'' function';
    return
end
...
for n = 1:Nrcvrs
    for m = 1:Nexp
        for q = 1:Naverages
            ...
            if p == 1
                sz = size(Im0);
                ImArr = zeros([sz Nexp Naverages],'single');                 % update to complex here???
            end
            ImArr(:,:,:,m,q) = Im0;
            clear Im0;
            p = p+1;
    ...
    if Nrcvrs > 1
        ...
        if n == 1
            ...
            ImHighSoS = complex(zeros(x,y,z,Nexp,Naverages,'single'),zeros(x,y,z,Nexp,Naverages,'single'));
        end
...
if Nrcvrs > 1
    ...
    %-------------------------------------------  
    Im = complex(zeros(x,y,z,Nexp,Naverages,'single'),zeros(x,y,z,Nexp,Naverages,'single'));
    for m = 1:Nexp
        for q = 1:Naverages
