...
function [FID,err] = ImportFIDV_SPGR3dShim_v1b_Func(FID,INPUT)
...
for n = 1:nrcvrs
    if DataInfo.maxvals(n) < 0.8*median(DataInfo.maxvals)
        button = questdlg(['Gain on rcvr',num2str(n),' is potentially low'],'Receive Gain Error','continue','abort','continue');
        if strcmp(button,'abort')
            err.flag = 1;
