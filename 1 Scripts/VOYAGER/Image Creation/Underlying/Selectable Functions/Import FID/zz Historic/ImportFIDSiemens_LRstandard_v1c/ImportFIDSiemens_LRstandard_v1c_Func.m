...
function [FID,err] = ImportFIDSiemens_LRstandard_v1c_Func(FID,INPUT)
...
%---------------------------------------------
sz = size(FIDmat);
if isfield(IMP,'WRTRCN')
    WRTRCN = IMP.WRTRCN;
    nproj = length(WRTRCN.projsampscnr);
else
...
if strcmp(FID.fovadjust,'Yes')
    ...
    shift(2) = -shift(2);
    shift(3) = -shift(3);
    KArr = KMat2Arr(IMP.Kmat(WRTRCN.projsampscnr,:,:),nproj,IMP.PROJimp.npro);
    for n = 1:sz(3)
        for m = 1:sz(4)
            FIDarr = DatMat2Arr(FIDmat(:,:,n,m),nproj,IMP.PROJimp.npro);
            FIDarr = FIDarr.*exp(-1i*2*pi*shift*KArr.').';
            FIDmat(:,:,n,m) = DatArr2Mat(FIDarr,nproj,IMP.PROJimp.npro);
        end
    end
end
