...
function [FID,err] = ImportFID_SiemensYB_v2g_Func(FID,INPUT)
...
if not(isempty(FID.rcvrtest))
    FIDmat0 = DATA.FIDmat(:,:,:,FID.rcvrtest,:,:);
else
    FIDmat0 = DATA.FIDmat;
end
...
% Do Averging
%---------------------------------------------
if isfield(FID,'doaveraging')
    if strcmp(FID.doaveraging,'Yes')
