...
function [FID,err] = ImportFIDV_SPGR3d_v1a_Func(FID,INPUT)
...
params = {'np','nv1','nv2','fovro','fovpe1','fovpe2','voxro','voxpe1','voxpe2',...
          'rcvrs','rosamp','rfcoil','gcoil',...
          'Efa','Epw','Epat','Etpwr','tr','te','tro','fildel','graddel','orp',...
...
% Sort
%-------------------------------------------
FIDmat = zeros(np,nv1,nv2,nrcvrs);
for m = 1:nv2
    for n = 1:nv1
...
if sz(1)~= np || sz(2)~= nv1 || sz(3)~= nv2 || sz(4)~= nrcvrs
    err.flag = 1;
...
if strcmp(FID.visuals,'Yes');
    for n = 1:nrcvrs
        figure(2000); hold on;
        plot(squeeze(abs(FIDmat(np/2,nv1/2,:,n))));
        plot(squeeze(abs(FIDmat(np/2,:,nv2/2,n))));
        title('Phase Encode Centre Test');
        xlabel('Phanse Encode Number');      

        tFIDmat = squeeze(FIDmat(:,:,:,n));
...
for n = 1:nrcvrs
    tFIDmat = FIDmat(:,:,:,n);
    MaxRealVal(n) = max(real(tFIDmat(:)));
    MaxImagVal(n) = max(imag(tFIDmat(:)));
    MaxAbsVal(n) = max(abs(tFIDmat(:)));
end
%MaxRealVal
%MaxImagVal
%MaxAbsVal
...
n = n+1;
Panel(n,:) = {'RFdur (us)',ExpPars.Epw,'Output'};
n = n+1;
Panel(n,:) = {'RFshape',ExpPars.Epat,'Output'};
n = n+1;
Panel(n,:) = {'','','Output'};
n = n+1;
Panel(n,:) = {'rosamp',ExpPars.rosamp,'Output'};
n = n+1;
Panel(n,:) = {'gcoil',ExpPars.gcoil,'Output'};
n = n+1;
Panel(n,:) = {'rfcoil',ExpPars.rfcoil,'Output'};
n = n+1;
Panel(n,:) = {'rfpwr',ExpPars.Etpwr,'Output'};
n = n+1;
Panel(n,:) = {'','','Output'};
