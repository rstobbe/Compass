...
function [SCRPTipt,SCRPTGBL,FID,err] = ImportFIDM_Shim_v1a(SCRPTipt,SCRPTGBL,FIDipt,RWSUI)
...
if not(isempty(val)) && val ~= 0
    ...
    if isfield(Gbl,'AutoRecon')
        ...
        FID.file = [Gbl.SaveName,'.MRD'];
        FID.shim = [FID.path,Gbl.ShimFile];
        FID.shimname = Gbl.ShimFile;
        loc = [FID.path,FID.file];
        label = loc;
        ...
        auto = 1;
        SCRPTGBL.([CallingLabel,'_Data']).('FIDpath_Data').path = FID.path;
        SCRPTGBL.([CallingLabel,'_Data']).('FIDpath_Data').file = FID.file;
    end
end
if auto ~= 1
    ...
    FID.file = FIDipt.([CallingLabel,'_Data']).('FIDpath_Data').file;
    shimname = uigetfile('*.SHM','Select Associated Shim',FID.path);
    if isempty(shimname)
        err.flag = 1;
        err.msg = '';
        return
    end
    FID.shim = [FID.path,shimname];
    FID.shimname = shimname;
end
