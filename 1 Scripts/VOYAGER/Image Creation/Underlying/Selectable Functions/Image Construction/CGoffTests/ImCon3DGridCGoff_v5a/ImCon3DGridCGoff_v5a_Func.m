...
function [GCGB0,err] = ImCon3DGridCGoff_v5a_Func(GCGB0,INPUT)
...
while true
    ...
    for its = 1:100
        ...
        % Create Output Filter
        %---------------------------------------------
        Filt = Kaiser_v1b(Imsz,Imsz,Imsz,beta(fnum),'unsym');         
        
        %---------------------------------------------
        % Filter Residual
        %---------------------------------------------
        Res = fftshift(fftn(ifftshift(Filt.*fftshift(ifftn(ifftshift(Res))))));      
