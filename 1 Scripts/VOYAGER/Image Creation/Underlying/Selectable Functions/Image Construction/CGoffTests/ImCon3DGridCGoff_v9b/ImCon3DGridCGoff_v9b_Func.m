...
function [GCGB0,err] = ImCon3DGridCGoff_v9b_Func(GCGB0,INPUT)
...
% FoV Mask Generate
%---------------------------------------------
Status2('busy','Generate FoV Mask',2);
...
%---------------------------------------------
Status2('busy','Generate Intensity Mask',2);
iMask = zeros(Imsz,Imsz,Imsz);
Ival = 0.2*mean(mean(mean(Im(C-5:C+5,C-5:C+5,C-5:C+5))));
for n = 1:Imsz
    for m = 1:Imsz
        for p = 1:Imsz
            if Im(n,m,p) >= Ival
...
%Im = Im.*Mask.*iMask;
Imscale = max(abs(Im(:)));
IMSTRCT.type = 'abs'; IMSTRCT.start = 35; IMSTRCT.step = 4; IMSTRCT.stop = 130; 
IMSTRCT.rows = 6; IMSTRCT.lvl = [0 Imscale]; IMSTRCT.SLab = 0; IMSTRCT.figno = 1000; 
...
%     AxialMontage_v2a(ImN,IMSTRCT);    
    ...
    %Im = Im.*Mask.*iMask; 
    IMSTRCT.type = 'abs'; IMSTRCT.start = 35; IMSTRCT.step = 4; IMSTRCT.stop = 130; 
    IMSTRCT.rows = 6; IMSTRCT.lvl = [0 Imscale]; IMSTRCT.SLab = 0; IMSTRCT.figno = 1003; 
    IMSTRCT.docolor = 0; IMSTRCT.ColorMap = 'ColorMap4'; IMSTRCT.figsize = [];
    AxialMontage_v2a(Im,IMSTRCT);    
