...
function [GCGB0,err] = ImCon3DGridCGoff_v3b_Func(GCGB0,INPUT)
...
while true
    ...
    %---------------------------------------------
    if fnum == 1
        Mask(abs(Im)<IMaskV*max(abs(Im(:)))) = 0;
    end    
    ...
    if fnum == 1
        beta = betastart;
        Filt = Kaiser_v1b(Imsz,Imsz,Imsz,beta,'unsym'); 
    else
        beta = beta*betarelstep;
        if beta < betastop
            break
        end
        Filt = Kaiser_v1b(Imsz,Imsz,Imsz,beta,'unsym'); 
    ...
    % Filter
    %---------------------------------------------
    Im = abs(fftshift(fftn(ifftshift(Filt.*fftshift(ifftn(ifftshift(Im)))))));   
    ...
    for its = 1:100
        ...
        CGerr = (r'*r)/(im0'*im0)
        if fnum == 1
            if CGerr > CGerr0*relCGerrbreak1
                stop = 1;
            end
        else
            if CGerr > CGerr0*relCGerrbreakN
                stop = 1;
            end            
        end
        CGerr0 = CGerr;
