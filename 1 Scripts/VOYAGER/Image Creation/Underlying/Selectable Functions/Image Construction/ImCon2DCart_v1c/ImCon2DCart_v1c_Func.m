...
function [IC,err] = ImCon2DCart_v1c_Func(IC,INPUT)
...
% Create Images
%---------------------------------------------
%if length(size(FIDmat)) < 4
%    error();                        % get matrix right on the import
%end
...
for m = 1:nslices
    for p = 1:nvols    
        for n = 1:nrcvrs
            Im(:,:,m,p,n) = fftshift(ifftn(ifftshift(FIDmat(:,:,m,p,n))));
        end
    end
end
