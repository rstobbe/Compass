...
function [GRD,err] = GridkSpace_LclKern2D_v1i_Func(GRD,INPUT)
...
elseif strcmp(GRD.implement,'CUDA')    
    ...
    if strcmp(GRD.precision,'Double')
        if strcmp(GRD.type,'real');
            [GrdDat,err] = mS2GCUDADoubleR2D_v4f(ZF,Kx,Ky,KERN,SampDat,CONV,StatLev,CUDAdevice);
        elseif strcmp(GRD.type,'complex');
            [GrdDat,err] = mS2GCUDADoubleC2D_v4f(ZF,Kx,Ky,KERN,SampDat,CONV,StatLev,CUDAdevice);
        end
