...
function [GRD,err] = GridkSpace_LclKern_v1i_Func(GRD,INPUT)
...
if strcmp(GRD.implement,'Mex')
    if strcmp(GRD.precision,'Double')
        if strcmp(GRD.type,'real');
            [GrdDat,err] = mS2GMexDoubleR_v3a(ZF,Kx,Ky,Kz,KERN,SampDat,CONV,StatLev);
        elseif strcmp(GRD.type,'complex');
            [GrdDat,err] = mS2GMexDoubleC_v3a(ZF,Kx,Ky,Kz,KERN,SampDat,CONV,StatLev);
        end
    elseif strcmp(GRD.precision,'Single')
        if strcmp(GRD.type,'real');
            [GrdDat,err] = mS2GMexSingleR_v3a(ZF,Kx,Ky,Kz,KERN,SampDat,CONV,StatLev);
        elseif strcmp(GRD.type,'complex');
            [GrdDat,err] = mS2GMexSingleC_v3a(ZF,Kx,Ky,Kz,KERN,SampDat,CONV,StatLev);
        end 
    end
elseif strcmp(GRD.implement,'CUDA')    
    ...
    elseif strcmp(GRD.precision,'Single')
        if strcmp(GRD.type,'real');
            [GrdDat,err] = mS2GCUDASingleR_v4f(ZF,Kx,Ky,Kz,KERN,SampDat,CONV,StatLev,CUDAdevice);
        elseif strcmp(GRD.type,'complex');
            error();
        end 
    end
