...
function [MASK,err] = TiltedSagMask_v1a_Func(MASK,INPUT)
...
set(figno,'WindowKeyPressFcn',@TiltedSagMask_v1a_KeyPress);
waitfor(figno);
...
% Update Panel
%---------------------------------------------
inds = strcmp('DispPost (mm)',{SCRPTipt.labelstr});
indnum = find(inds==1);
if length(indnum) > 1
...
inds = strcmp('DispAnt (mm)',{SCRPTipt.labelstr});
indnum = find(inds==1);
if length(indnum) > 1
    indnum = indnum(SCRPTGBL.RWSUI.scrptnum);
end
