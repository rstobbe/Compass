...
function [ALGN,err] = AlignMultiImages_v1a_Func(ALGN,INPUT)
...
for m = 2:ImArrayLen
    Status2('busy',['Image: ',num2str(m)],3);
    RegIm = zeros(sz(1:4));
    %JiggleIm = abs(IMG{m}.Im(:,:,:,1));
    JiggleIm = abs(IMG{m}.Im(:,:,:,1,1,1));
    tform = imregtform(JiggleIm,SpaceRef,StatIm,SpaceRef,'rigid',optimizer,metric);    
    %test = tform.T
    for n = 1:sz(4)
        %rRegIm = imwarp(real(IMG{m}.Im(:,:,:,n)),tform,'OutputView',SpaceRef);
        %iRegIm = imwarp(imag(IMG{m}.Im(:,:,:,n)),tform,'OutputView',SpaceRef);
        rRegIm = imwarp(real(IMG{m}.Im(:,:,:,n,1,1)),tform,'OutputView',SpaceRef);
        iRegIm = imwarp(imag(IMG{m}.Im(:,:,:,n,1,1)),tform,'OutputView',SpaceRef);
        RegIm(:,:,:,n) = rRegIm + 1i*iRegIm;
    end
    IMG{m}.Im = RegIm;
end
IMG{1}.Im = IMG{1}.Im(:,:,:,1,1,1);
...
ImageCombine = 1;
if ImageCombine == 1
    ImArr = zeros([size(RegIm) ImArrayLen]);
    for m = 1:ImArrayLen
        ImArr(:,:,:,m) = IMG{m}.Im;
    end
end    
