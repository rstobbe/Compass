...
function [SCRPTipt,SCRPTGBL,err] = B0Mapping_v1a(SCRPTipt,SCRPTGBL)
...
B0MAP.load = SCRPTGBL.CurrentScript.('Load');
...
if strcmp(B0MAP.load,'Local')
    ...
    B0MAP.impath = SCRPTGBL.('Image_File_Data').path;
...
if auto == 0;
    ...
    if not(isempty(ind))
        name0 = ['B0MAP_',IMG.name(5:end)];
    else
        name0 = 'B0MAP_';
    end
    if strcmp(B0MAP.load,'File')
