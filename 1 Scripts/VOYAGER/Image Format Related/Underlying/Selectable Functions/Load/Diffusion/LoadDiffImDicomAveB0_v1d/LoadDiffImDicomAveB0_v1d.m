%=========================================================
% (v1d)
%       - Loading update for generic dicom file naming
%=========================================================

function [SCRPTipt,IMAT,err] = LoadDiffImDicom_v1d(SCRPTipt,IMATipt)
...
if not(isfield(IMATipt,[CallingLabel,'_Data']))
    if isfield(IMATipt.('FirstDicom_File').Struct,'selectedfile')
        file = IMATipt.('FirstDicom_File').Struct.selectedfile;
        if not(exist(file,'file'))
            ...
            IMATipt.([CallingLabel,'_Data']).('FirstDicom_File_Data').file = file;
        end
    else
        err.flag = 1;
        err.msg = '(Re) Load First Dicom File';
...
IMAT.locm = IMATipt.([CallingLabel,'_Data']).('FirstDicom_File_Data').path;
