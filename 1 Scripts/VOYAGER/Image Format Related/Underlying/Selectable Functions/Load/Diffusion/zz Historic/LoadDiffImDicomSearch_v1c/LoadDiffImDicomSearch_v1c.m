%=========================================================
% (v1c)
%       - Output matrix order changed
%=========================================================

function [SCRPTipt,IMAT,err] = LoadDiffImDicomSearch_v1c(SCRPTipt,IMATipt)
...
if not(isfield(IMATipt,[CallingLabel,'_Data']))
    if isfield(IMATipt.('DicomFolder').Struct,'selectedfile')
        file = IMATipt.('DicomFolder').Struct.selectedfile;
        if not(exist(file,'file'))
            ...
            IMATipt.([CallingLabel,'_Data']).('DicomFolder_Data').file = file;
        end
    else
        err.flag = 1;
        err.msg = '(Re) Load First Dicom File';
...
IMAT.protocolname = IMATipt.('Protocol_Name');
...
IMAT.locm = IMATipt.([CallingLabel,'_Data']).('DicomFolder_Data').path;
