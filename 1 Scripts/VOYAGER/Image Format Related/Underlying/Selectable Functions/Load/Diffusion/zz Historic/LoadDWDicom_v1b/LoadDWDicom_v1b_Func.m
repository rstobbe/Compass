%=========================================================
% - Matrix Output:  (:,:,b-values,dirs,aves,slices)
% - b0 images are averaged (i.e. same b0 image for each 'dir' and 'ave'
%=========================================================

function [IMAT,err] = LoadDWDicom_v1b_Func(IMAT,INPUT)
...
% Load b0 > 0 Images
%-------------------------------------
dwims = zeros(dinfo.Rows,dinfo.Columns,numbvalues,totdirs,numaverages,totslices);
for slice = 1:totslices
    for nave = 1:numaverages
        for d = 1:totdirs        
            ...
            for a = 1:numbvalues
                if a == 1
                    dwims(:,:,1,d,nave,slice) = aveb0im(:,:,slice);
                else
                    if numres == 2
                        dwims(:,:,a,d,nave,slice) = dicomread([filem,'.',num2str(m,'%2.2i'),'.IMA']);
                    elseif numres == 3
                        dwims(:,:,a,d,nave,slice) = dicomread([filem,'.',num2str(m,'%2.3i'),'.IMA']);
                    elseif numres == 4
                        dwims(:,:,a,d,nave,slice) = dicomread([filem,'.',num2str(m,'%2.4i'),'.IMA']);
                    end
...
IMAT.dwims = dwims;
IMAT.iminfo = iminfo;
IMAT.dims = {'x','y','bvalue','dir','ave','slice'};
