...
% CV:  (nproj x npro)
%
%  - (v2a) was created for solzzving this in segments...
%==================================================

function [KrnVals] = SlvNghKrnVals_v2a(NGH,Kmat,Krn,iKrnRes,AIDrp,n,dpts) 
...
for i = 1:AIDrp.npro
    for j = 1:AIDrp.nproj 
        ...
        KrnVals{n} = Krn(round(abs(Kx(NGH{n}) - Kx(j,i))*iKrnRes)+1 + ...
                    round(abs(Ky(NGH{n}) - Ky(j,i))*iKrnRes)*szKrn(1) + ...
                    round(abs(Kz(NGH{n}) - Kz(j,i))*iKrnRes)*szKrn(1)*szKrn(2));
    end
    Status2('busy',['Solve Kernal Values - Trajectory Sample Number: ',num2str(i)],3);
end        
Status2('done','',3);
