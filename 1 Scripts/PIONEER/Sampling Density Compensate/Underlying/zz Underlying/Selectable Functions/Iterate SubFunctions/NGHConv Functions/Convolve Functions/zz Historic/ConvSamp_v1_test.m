...
function [CV,Ksz,Kx,Ky,Kz] = ConvSamp_v1_test(Ksz,Kx,Ky,Kz,DATR,W,beta,KrnRes,stat_tag)
...
for i = 1:length(Kxr)
	tic
    goodIndsX = uint32(find(abs(Kxr - Kxr(i)) <= 2*iKrnRes));           % search = 10x longer duration than convolution below
