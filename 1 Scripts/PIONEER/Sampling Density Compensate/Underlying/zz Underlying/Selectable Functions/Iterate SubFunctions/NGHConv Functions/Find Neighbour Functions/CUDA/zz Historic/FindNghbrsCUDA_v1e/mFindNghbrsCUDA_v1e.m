...
function [NGH,error,errorflag] = mFindNghbrsCUDA_v1e(Kmat,W,AIDrp) 
...
while go == 1
    if start > stop0    
        ...
        start0 = stop0+step;
        TstIn = 1;
        [~,~,test,error] = FindNghbrsCUDA_v1e(single(Kx),single(Ky),single(Kz),int32(Radinds),int32(npro),int32(nproj),...
                                                        single(W),int32(MaxXNghbrs),int32(MaxYNghbrs),int32(MaxZNghbrs),...
                                                        int32(Dpts),int32(Dptlen),int32(TstIn));
        if not(isempty(error))
            Status2('error',error,3);
            errorflag = 1;
            return;
    ...
    Dptlen = length(Dpts);
    TstIn = 0;
    [NGH0,NGH0lens,test2,error] = FindNghbrsCUDA_v1e(single(Kx),single(Ky),single(Kz),int32(Radinds),int32(npro),int32(nproj),...
                                                   single(W),int32(MaxXNghbrs),int32(MaxYNghbrs),int32(MaxZNghbrs),...
                                                   int32(Dpts),int32(Dptlen),int32(TstIn));
    MemOut = test2(5)
    if not(isempty(error))
