%================================================================================
% RootKernTest = CTF_Sphere_v1d
%================================================================================

function [CTF,SCRPTipt,err] = RootKernTest(PROJdgn,PROJimp,TF,KRNprms,CTF,SDCS,SCRPTipt,err)
...
[ConvTF0,outerror,outerrorflag] = mFCMexSingleR_v3(Ksz,Kx,Ky,Kz,KRNprms,Val,CONVprms,StatLev);
[Val2,~,~] = mG2SMexSingleR_v3(Ksz,Kx,Ky,Kz,KRNprms,ConvTF0,CONVprms,StatLev);  
...
rad2 = ((Kx-centre).^2 + (Ky-centre).^2 + (Kz-centre).^2).^0.5;
figure(100); hold on;
plot(rad2,Val2,'*');
%plot(ConvTF(:,centre,centre));
error('testing');
...
elseif strcmp(CTF.scale,'CentreVal1')
    ...
    ConvTFprof = ConvTFprof/(CentreVal*(SubSamp^3));
else
    ConvTFprof = ConvTFprof/(SubSamp^3);
end
