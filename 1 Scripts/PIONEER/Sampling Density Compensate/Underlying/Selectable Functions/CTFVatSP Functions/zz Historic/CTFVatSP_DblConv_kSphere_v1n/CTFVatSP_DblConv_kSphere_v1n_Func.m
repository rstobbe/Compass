...
function [CTFV,err] = CTFVatSP_DblConv_kSphere_v1n_Func(CTFV,INPUT)
...
% Functions
%---------------------------------------------
CTFV.S2GConvfunc = 'mS2GCUDADoubleR_v5b';
CTFV.G2SConvfunc = 'mG2SCUDADoubleR_v5b';
...
if (strcmp(CTFvis,'On'))         
    %figure(400); 
    %subplot(2,2,4); hold on;
    %plot((((Kx-centre).^2 + (Ky-centre).^2 + (Kz-centre).^2).^(0.5))/TFB.RadDim,DOV,'k*');              % too many points to show...
