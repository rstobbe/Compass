...
function [CTFV,err] = CTFVatSP_DblConv_kSphere_v1j_Func(CTFV,INPUT)
...
elseif strcmp(SDCS.Device,'CUDA')
    CUDAdevice = 1;
    if strcmp(SDCS.Precision,'Double')
        CTFV.S2GConvfunc = 'mS2GCUDADoubleR_v4e';
        CTFV.G2SConvfunc = 'mG2SCUDADoubleR_v4e';
    elseif strcmp(SDCS.Precision,'Single')
        CTFV.S2GConvfunc = 'mS2GCUDASingleR_v4e';
        CTFV.G2SConvfunc = 'mG2SCUDASingleR_v4e';
    end
end
...
%----------------------------------------
[Ksz,Kx,Ky,Kz,centre] = NormProjGrid_v4c(TFB.Loc,NaN,NaN,kstep,CONVprms.chW,TFB.SubSamp,'A2A');
Fcentre = centre;
%test = max([Kx,Ky,Kz]);
...
StatLev = 3;
[ConvTF,err] = S2Gconvfunc(Ksz,Kx,Ky,Kz,DblKern,TFB.Val,CONVprms,StatLev,CUDAdevice);
if err.flag
    return
end
...
StatLev = 3;
[ConvTF,err] = S2Gconvfunc(Ksz,Kx,Ky,Kz,FwdKern,TFB.Val,CONVprms,StatLev,CUDAdevice);
if err.flag
    return
end
...
%----------------------------------------
[Ksz,Kx,Ky,Kz,centre] = NormProjGrid_v4c(Kmat,PROJimp.nproj,PROJimp.npro,kstep,CONVprms.chW,TFB.SubSamp,'M2A');
...
[DOV,err] = G2Sconvfunc(Ksz,Kx,Ky,Kz,RvsKern,ConvTF,CONVprms,StatLev,CUDAdevice);
if err.flag == 1
    return
end
