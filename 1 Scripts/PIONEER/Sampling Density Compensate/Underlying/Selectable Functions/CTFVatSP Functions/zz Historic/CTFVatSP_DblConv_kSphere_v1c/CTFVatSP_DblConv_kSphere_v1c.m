%=========================================================
% (v1c)
%       - input change (SCRPTGBL added - to accommodate other function)
%=========================================================

function [DOV,SDCS,SCRPTipt,err] = CTFVatSP_DblConv_kSphere_v1c(Kmat,PROJimp,TF,KRNprms,SDCS,SCRPTipt,SCRPTGBL,err)
...
ConvTF = ConvTF/(DblKern.convscaleval*CTF.SubSamp^3);
...
if (strcmp(CTFvis,'On'))         
    ...
    plot([TF.r 1],[TF.tf 0]*PROJimp.projosamp*PROJimp.trajosamp,'k:');
end
...
ConvTF = ConvTF/(FwdKern.convscaleval*CTF.SubSamp^3);
ConvTF = ConvTF*PROJimp.projosamp*PROJimp.trajosamp;
...
DOV = DOV/(RvsKern.convscaleval*CTF.SubSamp^3);
%normDOV = DOV(1);
%DOV = DOV/normDOV;
if (strcmp(CTFvis,'On'))         
    figure(300); hold on;
    plot((((Kx-centre).^2 + (Ky-centre).^2 + (Kz-centre).^2).^(0.5))/CTF.RadDim,DOV,'k*');
end
