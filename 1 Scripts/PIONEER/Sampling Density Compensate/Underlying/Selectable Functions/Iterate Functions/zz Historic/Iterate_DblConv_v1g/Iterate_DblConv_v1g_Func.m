...
function [IT,err] = Iterate_DblConv_v1g_Func(IT,INPUT)
...
elseif strcmp(SDCS.Device,'CUDA')
    if strcmp(SDCS.Precision,'Double')
        IT.S2GConvfunc = 'mS2GCUDADoubleR_v4d';
        IT.G2SConvfunc = 'mG2SCUDADoubleR_v4d';
    elseif strcmp(SDCS.Precision,'Single')
        IT.S2GConvfunc = 'mS2GCUDASingleR_v4d';
        IT.G2SConvfunc = 'mG2SCUDASingleR_v4d';
    end
end
...
for j = iterations + 1:500   
    ...
    % Convolve
    %--------------------------------------
    [CV,~] = S2Gconvfunc(Ksz,Kx,Ky,Kz,FwdKern,SDC0,FwdConvPrms,StatLev);                
