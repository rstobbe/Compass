...
function [IT,err] = Iterate_DblConv_v1e_Func(IT,INPUT)
...
%--------------------------------------
%maxkmax0 = PROJimp.maxrelkmax*PROJdgn.kmax;
maxkmax = max(sqrt(Kmat(:,npro,1).^2 + Kmat(:,npro,2).^2 + Kmat(:,npro,3).^2));
kernwid = FwdKern.W;
...
for j = 1:500   
    ...
    INPUT.j = j;
    [ANLZ,err] = anlzfunc(IT.ANLZ,INPUT); 
    if err.flag
        return
    end
    clear INPUT
    ...
    INPUT.E = E;
    INPUT.j = j;
    [BRK,err] = breakfunc(IT.BRK,INPUT);
    if err.flag
        return
    end
    clear INPUT
