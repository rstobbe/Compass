...
function [IT,err] = Iterate_DblConv2D_v1h_Func(IT,INPUT)
...
if size(Kmat,2) == 3 
    if sum(Kmat,3) ~= 0
        err.flag = 1;
        err.msg = '''Iterate_DblConv2D'' is for 2D kSpace Sampling';
        return 
    end
end
...
elseif strcmp(SDCS.Device,'CUDA')
    ...
    if strcmp(SDCS.Precision,'Double')
        IT.S2GConvfunc = 'mS2GCUDADoubleR2D_v4f';
        IT.G2SConvfunc = 'mG2SCUDADoubleR2D_v4f';
...
%--------------------------------------
[Ksz,Kx,Ky,C] = NormProjGrid2D_v4c(Kmat,PROJimp.nproj,PROJimp.npro,PROJdgn.kstep,chW,SDCS.SubSamp,'M2A');

%--------------------------------------
% zero SDC beyond kmax
%--------------------------------------
r = sqrt(Kmat(:,:,1).^2 + Kmat(:,:,2).^2);
...
for j = iterations + 1:500   
    ...
    % Convolve
    %--------------------------------------
    [CV,~] = S2Gconvfunc(Ksz,Kx,Ky,FwdKern,SDC0,FwdConvPrms,StatLev,CUDAdevice);                
    CV = CV/(FwdKern.convscaleval);
    [W,~] = G2Sconvfunc(Ksz,Kx,Ky,RvsKern,CV,RvsConvPrms,StatLev,CUDAdevice);          
    W = W/(RvsKern.convscaleval*SDCS.SubSamp^2);
