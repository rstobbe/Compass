...
while true    
    ...
    while true
        ...
        INPUT.PSF = PSF;
        INPUT.PSD = PSD;
        [meov0,ANLZout] = MEOVatTroSnrArr_v1a_Reg(lb,ANLZ,INPUT);
        if meov0 == 0
            lb = lb+0.025;
        else
            break
    ...
    INPUT.func = 'FindMeov';
    for p = 1:4
        [meov(m,p),ANLZoutArr(m,p)] = MEOVatTroSnrArr_v1a_Reg(V(p),ANLZ,INPUT);
    ...
    %---------------------------------------------
    for p = 1:length(iV)
        [meov(m,p+4),ANLZoutArr(m,p+4)] = MEOVatTroSnrArr_v1a_Reg(iV(p),ANLZ,INPUT);
