...
function [SCRPTipt,SCRPTGBL,err] = OB_IRS_instepVOImm_relationship_v1a(SCRPTipt,SCRPTGBL)
...
instep = SCRPTGBL.CurrentTree.('InStep');

ind = strfind(instep,':');
if isempty(ind)
    ANLZ.instep = str2double(instep);
else
    start = str2double(instep(1:ind(1)-1));
    step = str2double(instep(ind(1)+1:ind(2)-1));
    stop = str2double(instep(ind(2)+1:end));
    ANLZ.instep = (start:step:stop);
end
