...
function [ANLZ,err] = MEOVatMeNeiSnrArr_v1a_Func(ANLZ,INPUT)
...
dir = 1;
while true
    [meov0,ANLZout] = MEOVatMeNeiSnrArr_v1a_Reg(lb,ANLZ,INPUT);
    if dir == 1 && meov0 == 0
        lb = lb+0.5;
...
INPUT.func = 'FindMeov';
for p = 1:4
    [meov(p),ANLZoutArr(p)] = MEOVatMeNeiSnrArr_v1a_Reg(V(p),ANLZ,INPUT);
...
%---------------------------------------------
for p = 1:length(iV)
    [meov(p+4),ANLZoutArr(p+4)] = MEOVatMeNeiSnrArr_v1a_Reg(iV(p),ANLZ,INPUT);
