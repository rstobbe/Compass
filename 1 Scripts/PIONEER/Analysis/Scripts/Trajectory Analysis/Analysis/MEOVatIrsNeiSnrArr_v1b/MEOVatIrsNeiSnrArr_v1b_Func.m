...
function [ANLZ,err] = MEOVatIrsNeiSnrArr_v1b_Func(ANLZ,INPUT)
...
dir = 1;
while true
    [meov0,ANLZout] = MEOVatIrsNeiSnrArr_v1b_Reg(lb,ANLZ,INPUT);
    if dir == 1 && meov0 == 0
        lb = lb+0.5;
...
INPUT.func = 'FindMeov';
for p = 1:4
    [meov(p),ANLZoutArr(p)] = MEOVatIrsNeiSnrArr_v1b_Reg(V(p),ANLZ,INPUT);
...
%---------------------------------------------
for p = 1:length(iV)
    [meov(p+4),ANLZoutArr(p+4)] = MEOVatIrsNeiSnrArr_v1b_Reg(iV(p),ANLZ,INPUT);
...
Panel(8,:) = {'fROI at Best SNR',fROIatBest,'Output'};
Panel(9,:) = {'Best SNR',BestSNR,'Output'};
