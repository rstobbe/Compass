...
function [ANLZ,err] = MEOVatIrsNeiSnrArr_v1a_Func(ANLZ,INPUT)

Status('busy','Calculate MEOV @ IRS and rNEI across SNR Array');
Status2('done','',2);
Status2('done','',3);
...
dir = 1;
while true
    [meov0,ANLZout] = MEOVatIrsNeiSnrArr_v1a_Reg(lb,ANLZ,INPUT);
    if dir == 1 && meov0 == 0
        lb = lb+0.5;
...
INPUT.func = 'FindMeov';
for p = 1:4
    [meov(p),ANLZoutArr(p)] = MEOVatIrsNeiSnrArr_v1a_Reg(V(p),ANLZ,INPUT);
    snr(p) = ANLZoutArr(p).snr;
end
figure(100); clf; hold on;
isnr = logspace(log10(4),log10(1.6/ANLZ.rnei),ANLZ.solpnts);
iV = interp1(snr,V,isnr,'pchip','extrap');
plot(V,snr,'*');
plot(iV,isnr,'*');
...
%---------------------------------------------
for p = 1:length(iV)
    [meov(p+4),ANLZoutArr(p+4)] = MEOVatIrsNeiSnrArr_v1a_Reg(iV(p),ANLZ,INPUT);
...
Panel(3,:) = {'PSF',INPUT.PSF.name,'Output'};
Panel(4,:) = {'PSD',INPUT.PSD.name,'Output'};
Panel(5,:) = {'IRS',ANLZ.aveirs,'Output'};
Panel(6,:) = {'rNEI',ANLZ.rnei,'Output'};
Panel(7,:) = {'Min MEOV (mm3)',MinMeov,'Output'};
Panel(8,:) = {'Best SNR',BestSNR,'Output'};
