...
function [SAMP,err] = kSpaceSampleMultRcvr_Grd128_v2c_Func(INPUT,SAMP)
...
%---------------------------------------------
Dat = zeros(x,Nrcvrs);
%zfkMat = zeros(135,135,135);
%zfkMat = zeros(139,139,139);
%zfkMat = zeros(149,149,149);
zfkMat = zeros(79,79,79);
for n = 1:Nrcvrs
    %zfkMat(4:131,4:131,4:131) = fftshift(fftn(ifftshift(squeeze(mOb(:,:,:,n)))));  
    %zfkMat(6:133,6:133,6:133) = fftshift(fftn(ifftshift(squeeze(mOb(:,:,:,n)))));  
    zfkMat(8:71,8:71,8:71) = fftshift(fftn(ifftshift(squeeze(mOb(:,:,:,n)))));  
    %zfkMat(11:138,11:138,11:138) = fftshift(fftn(ifftshift(squeeze(mOb(:,:,:,n)))));  
