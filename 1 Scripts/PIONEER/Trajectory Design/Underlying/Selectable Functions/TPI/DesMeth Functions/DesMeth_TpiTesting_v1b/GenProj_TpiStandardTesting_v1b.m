...
KSA = zeros(length(phi0),length(DESOL.tau2)+1,3);
kArrX = zeros(length(phi0),length(DESOL.tau2)+1);
kArrY = zeros(length(phi0),length(DESOL.tau2)+1);
kArrZ = zeros(length(phi0),length(DESOL.tau2)+1);   
...
%------------------------------------------
tautot = [0 PROJdgn.p+tau2(1:length(tau2))];      % (plin) for negative differential solution - from [1-40] thesis 
