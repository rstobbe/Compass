...
function [SCRPTipt,PROJDIST,err] = pad2a(SCRPTipt,PROJDIST)

err.flag = 0;
T = str2double(SCRPTipt(strcmp('Randomizer',{SCRPTipt.labelstr})).entrystr);
PDVis = SCRPTipt(strcmp('ProjAngleDist Vis',{SCRPTipt.labelstr})).entrystr;
if iscell(PDVis)
    PDVis = SCRPTipt(strcmp('ProjAngleDist Vis',{SCRPTipt.labelstr})).entrystr{SCRPTipt(strcmp('ProjAngleDist Vis',{SCRPTipt.labelstr})).entryvalue};
end
...
for i = 1:ncones  
    ...
    if strcmp(PDVis,'On')
        ...
        for j = 1:length(theta)
            plot((rad)*sin(phi(i))*cos(theta(j)),(rad)*sin(phi(i))*sin(theta(j)),'*');
        end
    end
