...
function [RADEVFUN,p,tau1,tau2,plin] = DEst_Flower_v1a(PROJipt,DEST,visuals)
...
if visuals == 2 
    r = (0:0.01:1);
    figure(92); hold on; plot(r,RADEVFUN(r),'k-','linewidth',2); xlabel('Relative Radial Value'); ylabel('Radial Evolution Value');
end

%------------------------------------------
% Calculate Projection Lengths
%------------------------------------------
Status('busy','Calculating Trajectory Lengths');
...
%------------------------------------------
Status('busy','Create Timing Vector');
...
if strcmp(initstrght,'Yes')
    ...
    if  visuals == 2 
        figure(100); hold on; plot(tau(1:5),'b-','linewidth',2); xlabel('sample point'); ylabel('tau'); title('Timing Vector'); drawnow;
    end
    ...
    while cont == 1;
        ...
        if err < 1e-12
            if tau(2) < tau(1)
                if visuals == 2 
                    figure(100); hold on; plot(tau(1:5),'g-','linewidth',2); xlabel('sample point'); ylabel('tau'); title('Timing Vector'); drawnow;
                end
                T = T*0.9999;
                inc = 0;
                N = 0.8;
...
if  visuals == 2 
    figure(100); hold on; plot(tau(1:5),'r-','linewidth',2); xlabel('sample point'); ylabel('tau'); title('Timing Vector');
end
if tau(2) < tau(1)
    errorflag = 1;
