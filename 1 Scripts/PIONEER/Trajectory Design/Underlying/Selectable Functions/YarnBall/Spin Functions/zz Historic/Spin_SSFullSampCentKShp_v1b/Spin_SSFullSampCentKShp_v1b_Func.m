...
function [SPIN,err] = Spin_SSFullSampCentKShp_v1b_Func(SPIN,INPUT)
...
if strcmp(Vis,'Yes') 
    ...
    rradcen = (SPIN.CentDiam/2)/PROJdgn.rad;
...
% Return
%--------------------------------------------- 
SPIN.GblSamp = SPIN.AddShpEndVal;
...
addshpendval = SPIN.AddShpEndVal;
addshpstartval = SPIN.AddShpStartVal;
beta = SPIN.AddShpBeta;
rradcen = (SPIN.CentDiam/2)/PROJdgn.rad + SPIN.smoothcenshift;
spincen = SPIN.CentScale*PROJdgn.nproj;
spincen = spincen - addshpstartval;
reltranslen = SPIN.reltranslen;
slope = spincen/reltranslen;
...
%---------------------------------------------
% Create Global Spin Shape
%---------------------------------------------
shape = besseli(0,beta * sqrt(1 - R.^2));
shape = shape - shape(length(shape));
shape = shape/shape(1);
shape = (addshpstartval-addshpendval)*shape+addshpendval;
prof = prof + shape;
...
% Smooth
%--------------------------------------------- 
proftemp = [prof(1)*ones(1,2500) prof prof(length(prof))*ones(1,2500)];
