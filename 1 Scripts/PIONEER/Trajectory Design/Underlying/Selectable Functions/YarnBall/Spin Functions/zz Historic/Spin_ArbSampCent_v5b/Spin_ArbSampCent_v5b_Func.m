...
function [SPIN,err] = Spin_ArbSampCent_v5b_Func(SPIN,INPUT)
...
% Smooth
%--------------------------------------------- 
if strcmp(SPIN.smoothcorners,'Yes')
    proftemp = [ones(1,5000) prof];
    profft = fftshift(fft(ifftshift(proftemp)));
    %filt = [zeros(7425,1);kaiser(151,SPIN.smoothbeta);zeros(7425,1)];
    filt = [zeros(7400,1);kaiser(201,SPIN.smoothbeta);zeros(7400,1)];
    %filt = [zeros(7375,1);kaiser(251,SPIN.smoothbeta);zeros(7375,1)];
    proftemp = fftshift(ifft(ifftshift(filt'.*profft)));
    prof = proftemp(5001:length(proftemp));
    %prof(R < rradcen/2) = PROJdgn.nproj/spincen;
    prof(R > rradcen + reltranslen*2) = PROJdgn.nproj/spingbl;
end
