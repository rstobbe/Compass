...
Panel(2,:) = {'EffSampDens',SPIN.GblSamp,'Output'};
Panel(3,:) = {'SampDensStart',SPIN.sampstart,'Output'};
Panel(4,:) = {'SampDensSEnd',SPIN.sampend,'Output'};
...
for n = 1:length(r)
    if r(n) < SPIN.sampshift
        Profile(n) = PROJdgn.nproj/SPIN.sampstart;
    else
        Profile(n) = PROJdgn.nproj./((SPIN.sampstart-SPIN.sampend).*Kaiser(r(n)-SPIN.sampshift) + SPIN.sampend);
    end
end
