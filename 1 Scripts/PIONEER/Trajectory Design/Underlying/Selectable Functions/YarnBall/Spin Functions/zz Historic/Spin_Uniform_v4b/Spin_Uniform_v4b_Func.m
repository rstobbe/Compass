...
function [SPIN,err] = Spin_Uniform_v4b_Func(SPIN,INPUT)
...
if strcmp(SPIN.ForceOddSpokes,'Yes')
    SPIN.nspokes = floor(sqrt(dproj*2)/2)*2+1;
else
    SPIN.nspokes = round(sqrt(dproj*2));
end
SPIN.nproj = SPIN.ndiscs*SPIN.nspokes;
dproj = SPIN.nproj;
SPIN.AziSampUsed = SPIN.ndiscs/sqrt(dproj/2)*SPIN.AziSamp;
SPIN.PolSampUsed = SPIN.nspokes/sqrt(dproj*2)*SPIN.PolSamp;
...
%---------------------------------------------
SPIN.spincalcnprojfunc = @(r) dproj/SPIN.PolSamp;
SPIN.spincalcndiscsfunc = @(r) sqrt((dproj/SPIN.AziSamp)/2);
