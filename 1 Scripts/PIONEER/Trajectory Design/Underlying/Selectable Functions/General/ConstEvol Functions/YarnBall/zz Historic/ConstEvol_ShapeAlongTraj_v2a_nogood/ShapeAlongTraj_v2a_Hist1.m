...
function CalcDeflectionAngle(obj)
    ...
    obj.DefAngle = [0;0;atan2(NormCross.',Dot)];
...
function EstimateInitVelAcc(obj)
    for n = 2:length(obj.kArr)-1        
        tSeg = obj.tArr(n+1)-obj.tArr(n);         
        dR(n) = obj.kMagArr(n+1) - obj.kMagArr(n);
        dA(n) = obj.PolarAngleRad(n);          

        dRdt(n) = dR(n)/tSeg;
        dRdt2(n) = (dRdt(n)-dRdt(n-1))/tSeg;
        dRdt3(n) = (dRdt2(n)-dRdt2(n-1))/tSeg;
        dAdt(n) = dA(n)/tSeg;
        dAdt2(n) = (dAdt(n)-dAdt(n-1))/tSeg;
        dAdt3(n) = (dAdt2(n)-dAdt2(n-1))/tSeg;        
        
        VelRad(n) = dRdt(n);
        VelCirc(n) = obj.kMagArr(n)*dAdt(n);        
        obj.Vel0(n) = sqrt(VelRad(n)^2 + VelCirc(n)^2);  
        
        AccRad(n) = dRdt2(n) - obj.kMagArr(n)*dAdt(n)^2;
        AccCirc(n) = obj.kMagArr(n)*dAdt2(n) + 2*dRdt(n)*dAdt(n);   
        obj.Acc0(n) = sqrt(AccRad(n)^2 + AccCirc(n)^2);  
        
        JrkRad(n) = dRdt3(n) - 3*obj.kMagArr(n)*dAdt(n)*dAdt2(n) - 3*dRdt(n)*dAdt(n)^2;
        JrkCirc(n) = 3*dRdt2(n)*dAdt(n) + 3*dRdt(n)*dAdt2(n) + obj.kMagArr(n)*dAdt3(n) - obj.kMagArr(n)*dRdt(n)^3;
        obj.Jrk0(n) = sqrt(JrkRad(n)^2 + JrkCirc(n)^2);          
    end
end  

%---------------------------------------------
% EstimateInitVelAccPolar (bad - uses deflaction-angle)
%---------------------------------------------
% function EstimateInitVelAccPolar(obj)
%     for n = 2:length(obj.kArr)
%         DistVec = obj.kArr(n,:)-obj.kArr(n-1,:);
%         MagDist = sqrt(DistVec(1)^2 + DistVec(2)^2 + DistVec(3)^2);
%         Angle = pi*obj.DefAngle(n)/180;
%         tSeg = obj.tArr(n)-obj.tArr(n-1); 
%         dRad(n) = MagDist/tSeg;
%         ddRad(n) = (dRad(n)-dRad(n-1))/tSeg;
%         dAngle(n) = Angle/tSeg;
%         ddAngle(n) = (dAngle(n)-dAngle(n-1))/tSeg;
%         VelRad(n) = dRad(n);
%         VelCirc(n) = obj.kMagArr(n)*dAngle(n);        
%         obj.Vel0(n) = sqrt(VelRad(n)^2 + VelCirc(n)^2);  
%         AccRad(n) = ddRad(n) - obj.kMagArr(n)*dAngle(n)^2;
%         AccCirc(n) = obj.kMagArr(n)*ddAngle(n) + 2*dRad(n)*dAngle(n);   
%         obj.Acc0(n) =  sqrt(AccRad(n)^2 + AccCirc(n)^2);      
%     end
% end 

%---------------------------------------------
% EstimateInitVelAccCart (doesn't work)
%---------------------------------------------
% function EstimateInitVelAccCart(obj)
% tVel = zeros(size(obj.kArr));
% tAcc = zeros(size(obj.kArr)); 
% tJrk = zeros(size(obj.kArr)); 
% tDist = zeros(size(obj.kArr));     
%     for n = 2:length(obj.kArr)
%         DistVec = obj.kArr(n,:)-obj.kArr(n-1,:);
%         tSeg = obj.tArr(n)-obj.tArr(n-1); 
%         for m = 1:3
%             tJrk(n,m) = (DistVec(m) - tVel(n-1,m)*tSeg - (1/2)*tAcc(n-1,m)*tSeg^2)*6/tSeg^3; 
%             tJrk0 = tJrk(n,:);
%             tAcc(n,m) = tAcc(n-1,m) + tJrk(n,m)*tSeg;
%             tAcc0 = tAcc(n,:);
%             tVel(n,m) = tVel(n-1,m) + tAcc(n-1,m)*tSeg + (tJrk(n,m)*tSeg^2)/2;
%             tVel0 = tVel(n,:);
%             tDist(n,m) = tVel(n-1,m)*tSeg + (1/2)*tAcc(n-1,m)*tSeg^2 + (1/6)*tJrk(n,m)*tSeg^3;
%             tDist0 = tDist(n,:); 
%             dif0 = tDist(n,m) - DistVec(m);
%             dif(n,m) = dif0;
%         end
%         test = max(abs(dif(:)))
%     end
...
function tSeg = SolveSpecifyAcc(obj,MaxAcc)
    ...
    tSeg0 = min(tSegRoots);
    tSeg01 = AdjustSegSpecifyAcc(obj,MaxAcc,tSeg0);
    tSeg = AdjustSegSpecifyAcc(obj,MaxAcc,tSeg01/2);  
    if round(tSeg01*1e8) ~= round(tSeg*1e8)
        test = 0;
    end
    obj.CalcVelAccJrk(tSeg);
    %obj.CalcDerivTinyDeflect(tSeg);
