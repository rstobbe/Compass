...
function tSeg = SolveSpecifyAcc(obj,MaxAcc)
    ...
    tSeg0 = min(tSegRoots);
    if strcmp(obj.AccFlag,'Mag')
        tSeg = AdjustSegSpecifyAcc(obj,MaxAcc,tSeg0);
        obj.CalcVelAccJrk(tSeg);
        if obj.AccCirc(obj.Loc) > 0.99*MaxAcc
            obj.AccFlag = 'Circ';
        end 
    elseif strcmp(obj.AccFlag,'Circ')
        tSeg = AdjustSegSpecifyAccCirc(obj,MaxAcc,tSeg0);
        obj.CalcVelAccJrk(tSeg);
    end    
end
...
%---------------------------------------------
% AdjustSegSpecifyAccCirc
%---------------------------------------------
function tSeg = AdjustSegSpecifyAccCirc(obj,AccVal,tSeg0)    
    rGradStep = 0.5;
    P = 8;                          % Grad search step
    its = 0;
    CalcVelAccJrk(obj,tSeg0);
    Acc0 = obj.AccCirc(obj.Loc); 
    while true
        if AccVal < 1e-5
            if abs(Acc0) < 1.0001*abs(AccVal) && abs(Acc0) > 0.9999*abs(AccVal)
                tSeg = tSeg0;
