...
function [CACC,err] = ConstEvol2D_v1a_Func(CACC,INPUT)
...
%---------------------------------------------
CACC.calcvelfunc = 'CalcVel2D_v2a';
CACC.calcaccfunc = 'CalcAcc2D_v2a';
CACC.calcjerkfunc = 'CalcJerk2D_v2a';
...
%---------------------------------------------
tro = PROJdgn.tro;
r = (sqrt(kArr(:,1).^2 + kArr(:,2).^2))/PROJdgn.kmax; 
...
magvel0 = sqrt(vel(:,1).^2 + vel(:,2).^2);
magacc0 = sqrt(acc(:,1).^2 + acc(:,2).^2);
magjerk0 = sqrt(jerk(:,1).^2 + jerk(:,2).^2);  
...
%irelr = (sqrt(ikArr(:,1).^2 + ikArr(:,2).^2))/PROJdgn.kmax;
...
magvel = sqrt(vel(:,1).^2 + vel(:,2).^2);
magacc = sqrt(acc(:,1).^2 + acc(:,2).^2);
magjerk = sqrt(jerk(:,1).^2 + jerk(:,2).^2);  
