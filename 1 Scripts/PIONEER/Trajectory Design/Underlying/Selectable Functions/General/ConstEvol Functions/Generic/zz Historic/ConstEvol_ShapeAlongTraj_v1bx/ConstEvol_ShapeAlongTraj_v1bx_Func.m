...
maxJrk = CACC.gacc*PROJimp.gamma;
%maxAcc = CACC.gvel*PROJimp.gamma;
...
% Visuals
%------------------------------------------
if strcmp(CACC.Vis,'Yes') && strcmp(ProfileTest,'Yes')
    %fh = figure(15); clf;
    fh = figure(15); 
...
if strcmp(CACC.Vis,'ProfOnly') && strcmp(ProfileTest,'Yes')
    %fh = figure(15); clf;
    fh = figure(15); hold on;
    fh.Name = 'Constrain Trajectory Evolution';
    fh.NumberTitle = 'off';
    fh.Position = [420 200 400 300];
    t = (0:0.001:tro) + Tacc(1);
    plot(t,Accproffunc(t)/max(Accproffunc(t)),'r-','linewidth',1); 
    xlim([0 t(end)]); ylim([0 1.2]);
    xlabel('ms'); ylabel('velocity_r_e_l'); title('Relative Gradient Velocity');
...
for n = 3:length(T)    
    ...
    if (rem(n,1) == 0 || n == length(T)) && strcmp(CACC.Vis,'Yes') 
        figure(15);
        subplot(2,2,1); hold on;
        ...
        plot(Tjrk,mJrk/PROJimp.gamma,'k');
        plot([0 Tjrk(end)],[maxJrk maxJrk]/PROJimp.gamma,'k:');
        ylim([0 1.3*maxJrk/PROJimp.gamma]);
