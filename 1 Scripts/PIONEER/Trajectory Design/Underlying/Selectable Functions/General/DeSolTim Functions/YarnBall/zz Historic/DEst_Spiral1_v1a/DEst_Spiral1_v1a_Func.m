...
function [DESOL,err] = DEst_Spiral1_v1a_Func(DESOL,INPUT)
...
%---------------------------------------------
DESOL.projlenoutfunc = 'Spiral1_ProjLenOutside_v1a';
DESOL.projleninfunc = 'Spiral1_ProjLenInside_v1a';
...
if strcmp(DESOL.Vis,'Yes') 
    r = (0:0.01:1);
    figure(2020); hold on; 
    plot(r,1./(r.*deradsolfunc(r)),'k-','linewidth',2); 
...
Status2('busy','Spiral Out',3);
[plout,err] = projlenoutfunc(p,deradsolfunc,RADEV.outtol);                    
if err.flag == 1
    return
end
Status2('busy','Spiral In',3);
RADEV.plout = plout;
[plin,err] = projleninfunc(p,deradsolfunc,RADEV);
if err.flag == 1
    return
end
Status2('done','',3);
...
if strcmp(TST.initstrght,'Yes')
    ...
    if strcmp(DESOL.Vis,'Yes') 
        ...
        xlabel('sample point'); ylabel('tau'); title('Timing Vector (Initial 5 points - want flat)'); drawnow;
    end
    ...
    while cont == 1;
        ...
        if n > 1000
            error('Timing Vector Problem (D param) - fix RadSolEvfunc');
        end
        err0val = errval;
    end
    SlvInd = ind;
...
if strcmp(DESOL.Vis,'Yes')
    ...
    xlabel('sample point'); ylabel('tau'); title('Timing Vector (Initial 5 points - want flat)');
