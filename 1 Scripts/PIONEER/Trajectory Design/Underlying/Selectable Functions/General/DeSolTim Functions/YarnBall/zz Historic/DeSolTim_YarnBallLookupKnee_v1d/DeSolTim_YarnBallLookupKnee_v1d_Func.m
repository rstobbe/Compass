...
function [DESOL,err] = DeSolTim_YarnBallLookupKnee_v1d_Func(DESOL,INPUT)
...
[plout,err] = ProjLenOutside(PROJdgn.p,IMPTYPE.MaxRadSolve,IMPTYPE.radevout);    
if IMPTYPE.MaxRadSolve > 1
    [plout0,err] = ProjLenOutside(PROJdgn.p,1,IMPTYPE.radevout); 
else
    plout0 = plout;
end
if err.flag == 1
...
if strcmp(RADEV.soltype,'basic')
    [plin,err] = ProjLenInsideRapid(PROJdgn.p,IMPTYPE.radevin,RADEV.intol);     
else
    [plin,err] = ProjLenInside(PROJdgn.p,IMPTYPE.radevin);
end
if err.flag == 1
    return
end
