...
function [CACCM,err] = CAccMeth4a_v1a_Func(CACCM,INPUT)
...
while true;
    ...
    Tseg(1) = Tsamp(2);
    [Tseg,Tsamp,Tvel,Vel,mVel,Tacc,Acc,mAcc,Tjrk,Jrk,mJrk,~] = CAccTop(3,Accproffunc,Tseg,Tsamp,Tvel,Vel,mVel,Tacc,Acc,mAcc,Tjrk,Jrk,mJrk,kArr,Accuracy,type);
...
for n = 3:length(T)    
    [Tseg,Tsamp,Tvel,Vel,mVel,Tacc,Acc,mAcc,Tjrk,Jrk,mJrk,Nreturn] = CAccTop(n,Accproffunc,Tseg,Tsamp,Tvel,Vel,mVel,Tacc,Acc,mAcc,Tjrk,Jrk,mJrk,kArr,Accuracy,type);
    Ntotarr(n) = Nreturn;   
...
% 
%=======================================================
function [Tseg,Tsamp,Tvel,Vel,mVel,Tacc,Acc,mAcc,Tjrk,Jrk,mJrk,Nreturn] = CAccTop(n,Accproffunc,Tseg,Tsamp,Tvel,Vel,mVel,Tacc,Acc,mAcc,Tjrk,Jrk,mJrk,kArr,Accuracy,type)
...
%             K1start = 3;
%             K2start = 3;
            Lvlstart = 3;                           % if Lvlstart == 3 -> can't make corner, allow flexibility
%             stepback = 1.1*stepback;
%             Tseg(n-1) = Tseg(n-2)/stepback;  
        end  
%     elseif Lvlstart == 3
