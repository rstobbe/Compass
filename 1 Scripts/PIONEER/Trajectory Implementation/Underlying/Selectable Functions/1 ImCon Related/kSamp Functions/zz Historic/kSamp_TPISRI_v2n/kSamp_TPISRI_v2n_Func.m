...
function [KSMP,err] = kSamp_TPISRI_v2n_Func(KSMP,INPUT)
...
if strcmp(gVis,'On')    
    ...
    for n = 1:length(TGSR)-1
        ...
        mGvis = [mGvis [mGSR(1,n,:) mGSR(1,n,:)]];
    end
    Gvis = squeeze(Gvis);
    mGvis = squeeze(mGvis);
    figure(1000); hold on; 
    plot(L,Gvis(:,1),'b-'); plot(L,Gvis(:,2),'g-'); plot(L,Gvis(:,3),'r-');
    plot(L,mGvis(:,1),'b:'); plot(L,mGvis(:,2),'g:'); plot(L,mGvis(:,3),'r:');
    drawnow;
    Gvis = []; mGvis = []; L = [];
    for n = 1:length(TGSR)-1
        L = [L [TGSR(n) TGSR(n+1)]];
        Gvis = [Gvis [GSR(round(nproj/4),n,:) GSR(round(nproj/4),n,:)]];
        mGvis = [mGvis [mGSR(round(nproj/4),n,:) mGSR(round(nproj/4),n,:)]];
    end
    Gvis = squeeze(Gvis);
    mGvis = squeeze(mGvis);
    figure(1001); hold on; 
    ...
    for n = 1:length(TGSR)-1
        ...
        mGvis = [mGvis [mGSR(nproj-1,n,:) mGSR(nproj-1,n,:)]];
    end
    Gvis = squeeze(Gvis);
    mGvis = squeeze(mGvis);
    figure(1002); hold on; 
