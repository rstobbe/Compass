...
function [IMPTYPE,err] = ImpType_ConesStd_v1i_Func(IMPTYPE,INPUT)
...
if strcmp(Func,'Generate')
    ...
    % Build Cones
    %---------------------------------------------
    DesGradPeriod = IMPTYPE.DesGradPeriod;          % in ms
    ...
    if Len >= 10000
        err.flag = 1;
        err.msg = 'Increase DesGradPeriod';
        return
    end
    ...
    while true
        ...
        for n = 1:round(IMPTYPE.SlvFrac*length(ThetaArr))
            ...
            title(['Cone Development Traj: ',num2str(n)]);
        ...
        if it == 1
            ...
            if IMPTYPE.ConesSpokes == 1
                break
            else
                osamp = 1 + (1-IMPTYPE.ConesSpokes)*(osamp-1);
            end
        elseif it == 2
            break
        end
    ...
    IMPTYPE.ncones = 2*length(ThetaArr);
    ...
    %---------------------------------------------
    if strcmp(TST.testing,'No')    
        ncones = length(ThetaArr);
        nprojcone = CeilNumIntlve;
        Rnd = 71;
        for i = 1:ncones  
            PhiStep(i) = 2*pi/nprojcone(i);
            if i == 1
                Phi = (2*pi-PhiStep(i)/2:-PhiStep(i):0.0001);
                CAPhi(i) = {Phi};
            elseif i == 2
                E = zeros(1,2*Rnd);
                for m = 1:2*Rnd
                    Phi = (2*pi-(m-1)*PhiStep(i)/(2*Rnd):-PhiStep(i):0.0001);
                    for k = 1:nprojcone(i)
                        ind1 = find(CAPhi{i-1} <= Phi(k),1,'first');
                        if isempty(ind1)
                            lval = CAPhi{i-1}(1) - 2*pi;
                        else
                            lval = CAPhi{i-1}(ind1);
                        end
                        ind2 = find(CAPhi{i-1} > Phi(k),1,'last');
                        if isempty(ind2)
                            gval = CAPhi{i-1}(length(CAPhi{i-1})) + 2*pi;
                        else
                            gval = CAPhi{i-1}(ind2);
                        end
                        E(m) = abs((gval - Phi(k)))^2 + abs((lval - Phi(k)))^2 + E(m);
                    end
                end
                ind = find(E == min(E),1,'first');
                Phi = (2*pi-(ind-1)*PhiStep(i)/(2*Rnd):-PhiStep(i):0.0001);
                CAPhi(i) = {Phi};
            else
                E = zeros(1,2*Rnd);
                for m = 1:2*Rnd
