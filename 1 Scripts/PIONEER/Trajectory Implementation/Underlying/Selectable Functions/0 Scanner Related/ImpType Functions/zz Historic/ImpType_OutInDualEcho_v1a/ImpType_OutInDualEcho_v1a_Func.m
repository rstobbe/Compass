...
function [IMPTYPE,err] = ImpType_OutInDualEcho_v1a_Func(IMPTYPE,INPUT)
...
elseif strcmp(INPUT.loc,'PostGeneration')
    ...
    zerotiming = (0:step:(nzeros-1)*step);
    
    %---------------------------------------------
    % Return
    %---------------------------------------------
    IMPTYPE.KSA = cat(2,KSA,flip(rKSA(:,1:end-1,:),2),zeroadd);
    IMPTYPE.T = [T T(end)+T(2:end) 2*T(end)+T(2)+zerotiming];
...
%     plot(RadTest(:,3),'b','linewidth',1);
    ...
    %---------------------------------------------       
    TSMP.troProt = TSMP.dwellProt*ceil(KSMP.Delay2Centre(2)/TSMP.dwellProt);
    TSMP.troMag = TSMP.troProt;
    TSMP.nproProt = TSMP.troProt/TSMP.dwellProt;
    TSMP.nproMag = TSMP.troMag/TSMP.dwell;
    IMPTYPE.TSMP = TSMP;
