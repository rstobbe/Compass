...
function [IMPTYPE,err] = ImpType_OutInOutMultiEcho_v1d_Func(IMPTYPE,INPUT)
...
elseif strcmp(INPUT.loc,'PostGeneration')
    ...
    % Return
    %---------------------------------------------
    IMPTYPE.KSA = cat(2,KSA,flip(rKSA(:,1:end-1,:),2),zeros(nproj,1,3),KSA);
    IMPTYPE.T = [T T(end)+T(2:end) 2*T(end)+T(2) 2*T(end)+T(2)+IMPTYPE.echo3delay/1000+T(2:end) 3*T(end)+2*T(2)+IMPTYPE.echo3delay/1000];
