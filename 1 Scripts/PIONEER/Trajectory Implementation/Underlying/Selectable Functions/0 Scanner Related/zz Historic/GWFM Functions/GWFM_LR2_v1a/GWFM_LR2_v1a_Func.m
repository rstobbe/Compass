...
function [GWFM,err] = GWFM_LR2_v1a_Func(GWFM,INPUT)
...
if strcmp(GWFM.Vis,'Yes') 
    ...
    xlabel('Time (ms)','fontsize',10,'fontweight','bold');
    ylabel('Gradients (mT/m)','fontsize',10,'fontweight','bold');
    figure(1700); hold on; plot(L,Gvis(TstTrj,:,1),'b:'); plot(L,Gvis(TstTrj,:,2),'g:'); plot(L,Gvis(TstTrj,:,3),'r:');
    plot(L,zeros(size(L)),'k:');
    title(['Traj',num2str(TstTrj),' (postInc)']);
    xlabel('Time (ms)','fontsize',10,'fontweight','bold');
    ylabel('Gradients (mT/m)','fontsize',10,'fontweight','bold');
end
...
if strcmp(GWFM.Vis,'Yes') 
    figure(5101); plot(qTwend(1:length(qTwend)-2),vecgsteps(1,:)/GQNT.gseg);
...
%----------------------------------------------------
% Include Uncompensated in Trajectory
%----------------------------------------------------
Status2('busy','Include Uncompensated in Trajectory',3);
func = str2func([GWFM.GINCfunc,'_Func']);
INPUT.PROJimp = PROJimp;
%INPUT.G0 = G0wend;
%INPUT.qT = qTwend;
INPUT.G0 = Gcomp;
INPUT.qT = qTcomp;
[GINC,err] = func(GINC,INPUT);
if err.flag
    return
end
Ginc = GINC.Ginc;
qTinc = GINC.Tinc;
GINC = rmfield(GINC,{'Ginc','Tinc'});
GWFM.GINC = GINC;
...
if strcmp(GWFM.Vis,'Yes') 
    ...
    figure(1700); hold on; plot(L,Gvis(TstTrj,:,1),'b-'); plot(L,Gvis(TstTrj,:,2),'g-'); plot(L,Gvis(TstTrj,:,3),'r-');
    figure(1500); hold on; plot(L,Gvis(TstTrj,:,1),'k-'); plot(L,Gvis(TstTrj,:,2),'k-'); plot(L,Gvis(TstTrj,:,3),'k-');
end
