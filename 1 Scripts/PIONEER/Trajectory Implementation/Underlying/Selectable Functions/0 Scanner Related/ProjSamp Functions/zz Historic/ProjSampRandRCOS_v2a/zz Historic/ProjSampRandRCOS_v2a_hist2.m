...
if SCRPTGBL.testing == 1
    ...
    for n = 1:totalprojs
        phistep = phi(n)-phi;
        inds01 = find(abs(phistep) < thphistep/3); 
        thetastep = theta(n) - theta(inds01);
        inds1 = find(abs(thetastep) < (minreldist/(sin(phi(n))*rad*osamptheta*p)));
        inds02 = find((abs(phistep) >= thphistep/3) & (abs(phistep) < 2*thphistep/3)); 
        thetastep = theta(n) - theta(inds02);
        inds2 = find(abs(thetastep) < (minreldist/(sin(phi(n))*rad*osamptheta*((1+p)/2))));
        inds03 = find(abs(phistep) >= 2*thphistep/3 & abs(phistep) < thphistep); 
        thetastep = theta(n) - theta(inds03);
        inds3 = find(abs(thetastep) < (minreldist/(sin(phi(n))*rad*osamptheta)));
