...
function [IMETH,err] = ImpMeth_LRstandard_v1d_Func(IMETH,INPUT)
...
if strcmp(mode,'TestTweak')
    ...
    Theta = 2*asin((MagkStep/2)./Rad);
    X = Rad.*cos(Theta/2);
    DeflectionErr = Rad-X;
        
    if strcmp(TVis,'Yes')
        fh = figure(500); clf;
        fh.Name = 'Solution Fineness Testing for Acceleration Constraint';
        fh.NumberTitle = 'off';
        ...
        plot3(PROJdgn.rad*KSA(1,ind2,1),PROJdgn.rad*KSA(1,ind2,2),PROJdgn.rad*KSA(1,ind2,3),'rx');
        ...
        plot([PROJdgn.p PROJdgn.p],PROJdgn.rad*[0 max(MagkStep)],':');
        xlabel('Relative Radius'); ylabel('kStep'); xlim([0 1]); title('Solution Sampling Fineness');

%         subplot(2,2,4); hold on;
%         plot(Rad,PROJdgn.rad*DeflectionErr,'k');
%         plot([PROJdgn.p PROJdgn.p],PROJdgn.rad*[0 max(DeflectionErr)],':');
%         xlabel('Relative Radius'); ylabel('DeflectionErr'); xlim([0 1]); 
        ...
        button = questdlg('Continue? (Test Solution Fineness for Acceleration Constraint)');
        if strcmp(button,'No');
            err.flag = 1;
            err.msg = 'aborted';
            return
...
if strcmp(mode,'Build')
    ...
    if strcmp(TVis,'Yes')
        ...
        plot3(PROJdgn.rad*GQKSA(1,ind2,1),PROJdgn.rad*GQKSA(1,ind2,2),PROJdgn.rad*GQKSA(1,ind2,3),'rx');
        lim = ceil(PROJdgn.rad*2*PROJdgn.p);
        xlim([-lim,lim]); ylim([-lim,lim]); zlim([-lim,lim]);
        xlabel('kStep_x'); ylabel('kStep_y'); zlabel('kStep_z');
        set(gca,'cameraposition',[-31 -39 12.5]); 
...
%         subplot(2,2,4); hold on;
%         plot(Rad,PROJdgn.rad*DeflectionErr,'b');
    ...
    if strcmp(GVis,'Yes') 
        figure(1000); subplot(2,2,3); hold on; 
        ...
        xlabel('(ms)','fontsize',10,'fontweight','bold');    
        plot(qTwend(2:length(qTwend)-2),maxcartgsteps/GQNT.gseg,'g-');
        figure(1000); subplot(2,2,4); hold on; 
        for p = 1:length(cartg2drv(1,:,1));
            maxcartg2drvT(p) = max(max(abs(cartg2drv(:,p,:))));
        end
        plot(qTwend(2:length(GQNT.scnrarr)-1),maxcartg2drvT(2:length(GQNT.scnrarr)-1)/GQNT.gseg^2,'g-');
...
if strcmp(mode,'Recon')
    ...
    if strcmp(KVis,'Yes') 
        ...
        figure(3000); subplot(2,1,1); hold on; 
        ...
        xlabel('Sampling Time (ms)'); ylabel('kSpace (1/m)')
        
        subplot(2,1,2); hold on;
        pts = 50;
        plot(KmatRecon(TstTrj,1:pts,1)/PROJdgn.kstep,'b*'); plot(KmatRecon(TstTrj,1:pts,2)/PROJdgn.kstep,'g*'); plot(KmatRecon(TstTrj,1:pts,3)/PROJdgn.kstep,'r*');    
        xlim([0 pts]);
        xlabel('Sampling Points'); ylabel('kSpace Steps')
    end    
    Kmat = KmatRecon;
    samp = SampRecon;
