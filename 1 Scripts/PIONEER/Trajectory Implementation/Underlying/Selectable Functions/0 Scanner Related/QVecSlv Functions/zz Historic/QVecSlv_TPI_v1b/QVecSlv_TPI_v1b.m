...
function [SCRPTipt,GQNTout,err] = QVecSlv_TPI_v1b(SCRPTipt,GQNT,TSMP)
...
%====================================================
    ...
    while true
        ...
        osamp = zeros(1,length(besttro));
        for n = 1:length(besttro)
            TSMP.PROJdgn.tro = besttro(n);
            [SCRPTipt,TSMPout,err] = sampfunc(SCRPTipt,TSMP);
            if err.flag
                ErrDisp(err);
                return
            end
            osamp(n) = TSMPout.trajosamp;    
        end
        if strcmp(GQNTout.slvprior1,'OvrSamp1');        
            osampbound = 1.05;
            trobound = 0.005;
            isegbound = 0.005;
            ind = find(osamp < TSMPout.oversamp0*osampbound);
            if not(isempty(ind))
                n = 0;
                for i = 1:length(ind)
                    if abs(besttro(ind(i))-PROJdgn.tro) < PROJdgn.tro*trobound
                        if abs(bestiseg(ind(i))-PROJdgn.iseg) < PROJdgn.iseg*isegbound
                            n = n+1;
                            good(n) = ind(i);
                        end
                    end
                end
                if strcmp(GQNTout.slvprior2,'OvrSamp2');   
                    good = good(find(osamp(good) == min(osamp(good)),1,'first'));        
                elseif strcmp(GQNTout.slvprior2,'Tro2');   
        ...
        elseif strcmp(GQNTout.slvprior1,'TwGseg1');        
            osampbound = 1.5;
            trobound = 0.005;
            isegbound = 0.005;
            twsegbound = 0.002;
            ...
            if not(isempty(ind))
                ...
                for i = 1:length(ind)
                    if abs(besttro(ind(i))-PROJdgn.tro) < PROJdgn.tro*trobound
                        if abs(bestiseg(ind(i))-PROJdgn.iseg) < PROJdgn.iseg*isegbound
                            if abs(((besttro(ind(i))-bestiseg(ind(i)))/twwords)-PROJimp.wantedtwseg) < PROJimp.wantedtwseg*twsegbound
                                n = n+1;
                                good(n) = ind(i);
                            end
                        end
