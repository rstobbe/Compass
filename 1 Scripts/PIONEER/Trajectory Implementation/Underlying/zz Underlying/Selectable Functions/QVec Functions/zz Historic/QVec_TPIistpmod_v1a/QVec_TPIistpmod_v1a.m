...
function [PROJimp,GQNT,IMPipt,err] = QVec_TPIistpmod_v1a(PROJdgn,PROJimp,GQNT,IMPipt,err)

twGsegiFracs = (IMPipt(strcmp('twGsegiFracs',{IMPipt.labelstr})).entrystr);
inds = strfind(twGsegiFracs,' ');
inds(length(inds)+1) = length(twGsegiFracs)+1;
fracs(1) = str2double(twGsegiFracs(1:inds(1)-1));
for n = 2:length(inds)
    fracs(n) = str2double(twGsegiFracs(inds(n-1):inds(n)-1));
end
...
GQNT.samparr = [0 (GQNT.iseg:GQNT.iseg:noisegs*GQNT.iseg)];
for n = 1:length(fracs)
    GQNT.samptwseg(n) = fracs(n)*GQNT.twseg;
    GQNT.samparr = [GQNT.samparr GQNT.samparr(length(GQNT.samparr))+GQNT.samptwseg(n)];
end
GQNT.samptwsegN = (GQNT.twseg * (twwords) - sum(GQNT.samptwseg))/(twwords - length(fracs));
GQNT.samparr = [GQNT.samparr GQNT.samparr(length(GQNT.samparr))+(GQNT.samptwsegN:GQNT.samptwsegN:wantedtro-GQNT.samparr(length(GQNT.samparr)))];
