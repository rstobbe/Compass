...
function [TST,err] = MpRageCnrOptT2sAnlz_v1c_Func(TST,INPUT)
...
for r = 1:length(TR)
    ...
    for k = 1:length(TIdel)
        for p = 1:length(Flip)
            ...
            MzDif = M(:,2)-M(:,1);
            MzDifValTI(p) = MzDif(TIseg(end));
            Mxy(p,:) = sin(pi*Flip(p)/180)*M(TIseg(end),:)*exp(-TE(r)/T2star);
            SigDifValTI(p) = abs(Mxy(p,2)-Mxy(p,1));
            SigDifValTI0(p) = abs(sin(pi*Flip(p)/180)*MzDifValTI(p));
        end
