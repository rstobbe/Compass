%====================================================
% (v3a)
%   - reflect underlying file naming change
%====================================================

function [FID1,FID2,expT,Params,ExpDisp,errorflag,error] = Load_2LocExperiment_v3a(path1,path2,Test,Sys,Seq,ExpDisp)
...
if strcmp(Sys,'Varian')
    if strcmp(Test,'PosLoc');
        [TP1,FID1,ParamsDisp,StudyDisp,errorflag,error] = LoadVarian_PosLoc_v1a(path1);
        if errorflag == 1
            return
        end
        [TP2,FID2,ParamsDisp,StudyDisp,errorflag,error] = LoadVarian_PosLoc_v1a(path2);
        if errorflag == 1
            return
        end
        [fs1,fs2,err] = comp_struct(TP1,TP2,'TP1','TP2');
        if not(isempty(err))
            errorflag = 1;
            error = 'Gradient files do not match';
            return
        else
            Params = TP1;
    ...
    if strcmp(Test,'Transient');
        ...
        [TP1,FID1,ParamsDisp,StudyDisp,errorflag,error] = func(path1);
        if errorflag == 1
            return
        end
        [TP2,FID2,ParamsDisp,StudyDisp,errorflag,error] = func(path2);
        if errorflag == 1
            return
        end
        [fs1,fs2,err] = comp_struct(TP1,TP2,'TP1','TP2');
