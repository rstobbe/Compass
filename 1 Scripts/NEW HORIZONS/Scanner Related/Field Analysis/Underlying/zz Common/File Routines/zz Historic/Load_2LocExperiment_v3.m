...
function [FID1,FID2,expT,Params,ExpDisp,errorflag,error] = Load_2LocExperiment_v3(path1,path2,Test,Sys,Seq,ExpDisp)
...
if strcmp(Sys,'Varian')
    ...
    if strcmp(Test,'Transient');
        ...
        if TP1.np~=TP2.np || TP1.dwell~=TP2.dwell || TP1.gval~=TP2.gval || length(TP1.gofftime)~=length(TP2.gofftime)
            errorflag = 1;
            error = 'files do not match';
            return
        else
    ...
    if strcmp(Test,'During');
        ...
        if TP1.np~=TP2.np || TP1.dwell~=TP2.dwell
            errorflag = 1;
            error = 'files do not match';
            return
        else
