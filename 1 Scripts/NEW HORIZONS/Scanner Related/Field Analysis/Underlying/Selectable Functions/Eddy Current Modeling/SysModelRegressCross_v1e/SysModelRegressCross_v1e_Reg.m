...
function E = SysModelRegressCross_v1e_Reg(V,ECM,INPUT)
...
%---------------------------------------------
% Sample Geddy at Centre of Step
%---------------------------------------------
csGeddy = Geddy(2:2:length(Geddy));
csTeddy = Teddy(2:2:length(Teddy));

%---------------------------------------------
% Find Error
%---------------------------------------------
%E = permute(csGmeas,[2 1]) - csGeddy(1:length(csGmeas));  
E = csGmeas - csGeddy(1:length(csGmeas)); 
%Esum = sum(abs(E))
...
plot(Teddy,Geddy,'c-');
plot(csTeddy,csGeddy,'c-');
title('Cross Transient Field Regression (c) from Applied Gradient (k)');
