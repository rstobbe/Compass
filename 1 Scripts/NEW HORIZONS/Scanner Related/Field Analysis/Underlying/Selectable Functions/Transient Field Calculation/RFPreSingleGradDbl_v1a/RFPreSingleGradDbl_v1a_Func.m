...
function [TF,err] = RFPreSingleGradDbl_v1a_Func(TF,INPUT)
...
% Calculate
%--------------------------------------------- 
OUTPUT(1) = CalcTransField(TF,POSBG(1),Sys,'a');
OUTPUT(2) = CalcTransField(TF,POSBG(2),Sys,'b');
TF.OUTPUT = OUTPUT;
...
% Calculate
%====================================================
function OUTPUT = CalcTransField(TF,POSBG,Sys,Case)

%-------------------------------------
% Determine Transient Fields
%-------------------------------------
[TF_Fid1,TF_Fid2,TF_Params,TF_ExpDisp,err] = Load_2LocExperiment_v4b(TF.(['FileGrad1',Case]),TF.(['FileGrad2',Case]),'RFPreSingleGrad_v1a',Sys);
if err.flag
    return
end
