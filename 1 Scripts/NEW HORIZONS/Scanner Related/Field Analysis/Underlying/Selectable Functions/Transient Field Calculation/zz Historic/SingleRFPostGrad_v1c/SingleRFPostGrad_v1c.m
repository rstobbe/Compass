...
function [SCRPTipt,TF,err] = SingleRFPostGrad_v1c(SCRPTipt,TFipt)
...
if not(isfield(TFipt,[CallingLabel,'_Data']))
    ...
    if isfield(TFipt.('File_PosLoc1').Struct,'selectedfile')
        file = TFipt.('File_PosLoc1').Struct.selectedfile;
        if not(exist(file,'file'))
            err.flag = 1;
            ...
            TFipt.([CallingLabel,'_Data']).('File_PosLoc1_Data').path = file;
        end
    else
        err.flag = 1;
        err.msg = '(Re) Load File_PosLoc1';
    ...
    if isfield(TFipt.('File_PosLoc2').Struct,'selectedfile')
        file = TFipt.('File_PosLoc2').Struct.selectedfile;
        if not(exist(file,'file'))
            err.flag = 1;
            ...
            TFipt.([CallingLabel,'_Data']).('File_PosLoc2_Data').path = file;
        end
    else
        err.flag = 1;
        err.msg = '(Re) Load File_PosLoc2';
