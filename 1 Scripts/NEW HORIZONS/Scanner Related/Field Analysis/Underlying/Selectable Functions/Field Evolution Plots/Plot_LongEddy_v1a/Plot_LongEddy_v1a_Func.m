...
function [PLOT,err] = Plot_LongEddy_v1a_Func(PLOT,INPUT)

Status2('busy','Plot Long Eddy Currents',2);
Status2('done','',3);
...
xlabel('(seconds)'); ylabel('Gradient Evolution (%)'); xlim([0 max(Time)]); title('Transient Field (Gradient)');
...
xlabel('(seconds)'); ylabel('B0 Evolution (uT)/(mT/m)'); xlim([0 max(Time)]); title('Transient Field (B0)');
