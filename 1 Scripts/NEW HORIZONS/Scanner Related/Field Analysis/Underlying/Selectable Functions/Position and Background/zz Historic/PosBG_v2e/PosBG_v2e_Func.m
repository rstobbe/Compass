...
function [POSBG,err] = PosBG_v2e_Func(POSBG,INPUT)
...
if maxPL_PH1step > 2.75 || maxPL_PH2step > 2.75
    figure(100); hold on; title('Phase Steps Too Large');
    xlabel('Acq (ms)')
    ylabel('Phase Steps (rad)')
    plot(PL_expT(PL_ind1:PL_ind2),PL_PH1steps,'r'); 
    plot(PL_expT(PL_ind1:PL_ind2),PL_PH2steps,'b');
    err.flag = 1;
    err.msg = 'Probable error with probe displacement or gradient magnitude';
    return
end
