...
function [POSBG,err] = PosBGcross_v2e_Func(POSBG,INPUT)
...
% Calculate
%--------------------------------------------- 
OUTPUT(1) = CalcPosBG(POSBG,Sys,'1A','2A');
OUTPUT(2) = CalcPosBG(POSBG,Sys,'1B','2B');
OUTPUT(3) = CalcPosBG(POSBG,Sys,'A1','B1');
OUTPUT(4) = CalcPosBG(POSBG,Sys,'A2','B2');
...
Panel(11,:) = {'LocA_1 (cm)',OUTPUT(3).Loc1*100,'Output'};
Panel(12,:) = {'LocB_1 (cm)',OUTPUT(3).Loc2*100,'Output'};
Panel(13,:) = {'Sep_1 (cm)',OUTPUT(3).Sep*100,'Output'};
Panel(14,:) = {'meanBGGrad_1 (uT/m)',OUTPUT(3).meanBGGrad*1000,'Output'};
Panel(15,:) = {'meanBGB0_1 (uT)',OUTPUT(3).meanBGB0*1000,'Output'};
Panel(16,:) = {'LocA_2 (cm)',OUTPUT(4).Loc1*100,'Output'};
Panel(17,:) = {'LocB_2 (cm)',OUTPUT(4).Loc2*100,'Output'};
Panel(18,:) = {'Sep_2 (cm)',OUTPUT(4).Sep*100,'Output'};
Panel(19,:) = {'meanBGGrad_2 (uT/m)',OUTPUT(4).meanBGGrad*1000,'Output'};
Panel(20,:) = {'meanBGB0_2 (uT)',OUTPUT(4).meanBGB0*1000,'Output'};
...
% Calculate
%====================================================
function OUTPUT = CalcPosBG(POSBG,Sys,Case1,Case2)

%-------------------------------------
% Load PosLoc Data
%-------------------------------------
[PL_Fid1,PL_Fid2,PL_Params,PL_ExpDisp,err] = Load_2LocExperiment_v4b(POSBG.(['FilePosLoc',Case1]),POSBG.(['FilePosLoc',Case2]),'PosLoc_v1a',Sys);
if err.flag
    return
end
...
%-------------------------------------
[BG_Fid1,BG_Fid2,BG_Params,BG_ExpDisp,err] = Load_2LocExperiment_v4b(POSBG.(['FileNoGrad',Case1]),POSBG.(['FileNoGrad',Case2]),'NoGrad_v1a',Sys);
if err.flag
    return
end
