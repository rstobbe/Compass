...
function [POSBG,err] = PosBgrndSmooth_v3d_Func(POSBG,INPUT)
...
initftBG_Bloc1 = ftBG_Bloc1(1:POSBG.smoothfraction/2*length(BG_Bloc1));
midftBG_Bloc1 = ftBG_Bloc1(POSBG.smoothfraction/2*length(BG_Bloc1)+1:end-POSBG.smoothfraction/2*length(BG_Bloc1));
endftBG_Bloc1 = ftBG_Bloc1(end-POSBG.smoothfraction/2*length(BG_Bloc1)+1:end);
...
initftBG_Bloc2 = ftBG_Bloc2(1:POSBG.smoothfraction/2*length(BG_Bloc2));
midftBG_Bloc2 = ftBG_Bloc2(POSBG.smoothfraction/2*length(BG_Bloc2)+1:end-POSBG.smoothfraction/2*length(BG_Bloc2));
endftBG_Bloc2 = ftBG_Bloc2(end-POSBG.smoothfraction/2*length(BG_Bloc2)+1:end);
...
freq = linspace(-1/(2*FEVOL.BG_Params.dwell),1/(2*FEVOL.BG_Params.dwell)-1/(length(ftBG_Bloc1)*FEVOL.BG_Params.dwell),length(ftBG_Bloc1));
plot(freq,fftshift(abs(ftBG_Bloc2)));
plot(freq,fftshift(abs(ftBG_smthBloc2)));
xlabel('kHz');
title('Loc2 FT & Smooth')

if max(imag(BG_smthBloc1(:))) > 1e-4 || max(imag(BG_smthBloc2(:))) > 1e-4
    test = max(imag(BG_smthBloc1(:)))
    test2 = max(imag(BG_smthBloc2(:)))
    error
end
