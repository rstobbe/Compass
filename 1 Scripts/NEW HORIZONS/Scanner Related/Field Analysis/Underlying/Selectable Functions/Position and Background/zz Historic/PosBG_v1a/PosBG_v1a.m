...
function [SCRPTipt,Output,err] = PosBG_v1a(SCRPTipt,SCRPTGBL,Input)

err.flag = 0;
Output = struct();

FileLocs = SCRPTGBL.FileLocs;
...
for w = 1:2
    ...
    %-------------------------------------
    ExpDisp = [];
    [PosLoc1,PosLoc2,expT,Params,ExpDisp,errorflag,error] = Load_2LocExperiment_v2(FileLocs.File_PosLoc1,FileLocs.File_PosLoc2,'PosLoc',Sys,'',ExpDisp);
    if errorflag == 1
        err.flag = 1;
        err.msg = error;
        return
    ...
    %-------------------------------------
    [BG1,BG2,expT,Params,ExpDisp,errorflag,error] = Load_2LocExperiment_v2(FileLocs.File_NoGrad1,FileLocs.File_NoGrad2,'BackGrad',Sys,'',ExpDisp);
    if errorflag == 1
        err.flag = 1;
        err.msg = error;
        return
