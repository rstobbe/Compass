...
function [FEVOL,err] = FieldEvoLoadSiemensMultiCoil_v1a_Func(FEVOL,INPUT)
...
for n = 1:2
    tData = Data{n};
    tBG_Fid = tData(:,UseCoil,GWFM.aves == 0,:);
...
for n = 1:2
    tData = Data{n};
    tPL_Fid = tData(:,UseCoil,GWFM.aves == 1,:);
...
for n = 1:2
    for m = 2:max(GWFM.aves)
        tData = Data{n};
        tTF_Fid = tData(:,UseCoil,GWFM.aves == m,:);
