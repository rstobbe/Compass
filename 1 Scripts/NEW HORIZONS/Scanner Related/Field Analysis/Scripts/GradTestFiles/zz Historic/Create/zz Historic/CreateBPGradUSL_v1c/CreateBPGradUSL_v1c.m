...
function [SCRPTipt,SCRPTGBL,err] = CreateBPGradUSL_v1c(SCRPTipt,SCRPTGBL)

Status('busy','Create Bipolar Gradient File');
Status2('done','',2);
Status2('done','',3);
...
G0 = [zeros(1,preglen) grampup GRD.gmax*ones(1,gatmaxlen) grampdown...
      zeros(1,interglen) -grampup -GRD.gmax*ones(1,gatmaxlen) -grampdown 0];
G = [G0;G0;G0]';
...
if round(GRD.graddur*1e9) ~= round(test*1e9)
    error();
end
...
WRTP.gval = GRD.gmax;
WRTP.gvalinparam = 0;
WRTP.slewrate = GRD.gsl;
WRTP.falltime = GRD.gmax/GRD.gsl;
WRTP.pnum = 1;
