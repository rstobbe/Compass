...
function [B0beta,B0fit,SCRPTipt] = B0_BiExp_wOffset(gofftime,B0eddy1,SCRPTipt,Clr,figno)
...
Est = [B0eddy1Sub(1)/2 0.3 B0eddy1Sub(1)/2 0.1 0];
...
[SCRPTipt] = AddToPanelOutput(SCRPTipt,'B0_offset (uT)',B0beta(5)*1000,'');

B0fit.subtime = gofftimeSub;
B0fit.subvals = B0beta(1)*(exp(-(gofftimeSub)/B0beta(2))) + B0beta(3)*(exp(-(gofftimeSub)/B0beta(4))) + B0beta(5);
B0fit.fulltime = gofftime;
B0fit.fullvals = B0beta(1)*(exp(-(gofftime)/B0beta(2))) + B0beta(3)*(exp(-(gofftime)/B0beta(4))) + B0beta(5);
