...
function [SCRPTipt,SCRPTGBL,err] = ExpRipModelFitting_v1a(SCRPTipt,SCRPTGBL)
...
RGRS.sincfreqest = str2double(SCRPTGBL.CurrentTree.('SincFreq_Est'));
RGRS.sinctcest = str2double(SCRPTGBL.CurrentTree.('SincTc_Est'));
RGRS.sincmagest = str2double(SCRPTGBL.CurrentTree.('SincMag_Est'));
tcest = SCRPTGBL.CurrentTree.('ExpTc_Est');
magest = SCRPTGBL.CurrentTree.('ExpMag_Est');
...
if isempty(inds1)
    ...
    for n = 2:length(inds1)
        ...
        RGRS.magest(n) = str2double(magest(inds2(n-1)+1:inds1(n)-1));
    end
    if isempty(n)
        n = 1;
    end
