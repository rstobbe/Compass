...
function [RGRS,err] = ShortEddyDecayFitting_v1a_Func(RGRS,INPUT)
...
% Centre of Fall
%-----------------------------------------------------
ind = find(GdesSub==0,1,'first');
centfall = TimeSub(ind)/2;
minGdesSub = min(GdesSub);
maxGdesSub = max(GdesSub);
...
if doplot == 1
    ...
    plot([centfall centfall],[minGdesSub maxGdesSub],'k:');
end
...
func = str2func([RGRS.regressfunc,'_Func']);
INPUT.Time = TimeSub - centfall;
INPUT.Gmeas = eddySub;
INPUT.Gmax = Gmax;
INPUT.fullTime = (0:0.01:INPUT.Time(length(INPUT.Time)));
...
if doplot == 1
    figure(5001); hold on;
    plot(INPUT.fullTime+centfall,ECM.Grgrs,'g-');
end
