...
function [SCRPTipt,SCRPTGBL,err] = Grad_MonoExpOffset_v2c(SCRPTipt,SCRPTGBL)
...
offset = str2double(SCRPTipt(strcmp('Offset (uT/m)',{SCRPTipt.labelstr})).entrystr);
...
if datastart == starttime
    starttime = timeSub(1);
end
timeSub = timeSub - timeSub(1) + starttime;
GeddySub = cGeddy(ind1:ind2);
...
[SCRPTipt] = AddToPanelOutput(SCRPTipt,'G_CompVal (V%)','0output',-100*gcal*Gbeta(1)/gval,'0numout');
...
plot(Gfit.interptime,Gfit.interpvals*1000+offset,[clr,'-'],'linewidth',2);
ylim([-max(abs(Geddy*1000)) max(abs(Geddy*1000))]);
xlabel('(ms)'); ylabel('G Evolution (uT/m)'); xlim([0 max(time)]); title('Transient Field (G)');
