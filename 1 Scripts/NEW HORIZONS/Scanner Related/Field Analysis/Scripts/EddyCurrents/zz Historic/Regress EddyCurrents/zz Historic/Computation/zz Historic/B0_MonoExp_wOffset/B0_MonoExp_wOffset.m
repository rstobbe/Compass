...
function [B0beta,B0fit,SCRPTipt] = B0_MonoExp_wOffset(gofftime,B0eddy1,SCRPTipt,Clr,figno)
...
[SCRPTipt] = AddToPanelOutput(SCRPTipt,'B0_timeconst1 (ms)',B0beta(2)*1000,'');
[SCRPTipt] = AddToPanelOutput(SCRPTipt,'B0_mag1 (uT)',B0beta(1)*1000,'');
[SCRPTipt] = AddToPanelOutput(SCRPTipt,'B0_offset (uT)',B0beta(3)*1000,'');

B0fit.subtime = gofftimeSub;
B0fit.subvals = B0beta(1)*(exp(-(gofftimeSub)/B0beta(2))) + B0beta(3);
B0fit.fulltime = gofftime;
B0fit.fullvals = B0beta(1)*(exp(-(gofftime)/B0beta(2))) + B0beta(3);
