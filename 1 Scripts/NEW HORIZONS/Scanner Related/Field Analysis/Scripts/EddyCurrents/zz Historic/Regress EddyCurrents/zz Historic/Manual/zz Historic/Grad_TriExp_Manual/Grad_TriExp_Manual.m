...
function [Gbeta,Gfit,SCRPTipt] = Grad_TriExp_Manual(gofftime,Geddy1,SCRPTipt,Clr,figno)
...
tc3 = str2double(SCRPTipt(strcmp('G_TC3 (ms)',{SCRPTipt.labelstr})).entrystr)/1000;
mag3 = str2double(SCRPTipt(strcmp('G_mag3 (mT/m)',{SCRPTipt.labelstr})).entrystr);

Gfit.fulltime = gofftime;
Gfit.fullvals = mag1*(exp(-(Gfit.fulltime)/tc1)) + ...
                 mag2*(exp(-(Gfit.fulltime)/tc2)) + ...
                 mag3*(exp(-(Gfit.fulltime)/tc3));
