...
function [CASL,err] = pCASL_Tagging3inhomo_v1f_Func(CASL,INPUT)
...
% Refocussing
%---------------------------------------------
Gave = CASL.gave + CASL.gfix;
refgrad = (G*CASL.rftau + Gave)/delay;                      
...
%---------------------------------------------
% Position Frequency Difference (ex-rephase)
%---------------------------------------------
delG = G + refgrad;
wposdif = 2*pi*gamma*delG*CASL.dist2iso;                % in rad/ms;

%---------------------------------------------
% Phase Increment
%---------------------------------------------
if strcmp(CASL.phaseinc,'auto')
    phaseinc = wposdif*delay;
else
    phaseinc = pi*str2double(CASL.phaseinc)/180;
end
phaseinc = phaseinc + pi*CASL.phasefix/180;
...
for n = 1:segments
    ...
    %---------------------------------------------
    % simulate RF phase shift
    %---------------------------------------------    
    phaseshift = phaseinc*(n-1);
    Mx = M(2);
    My = M(3);
    M(2) = Mx*cos(phaseshift) - My*sin(phaseshift);
    M(3) = Mx*sin(phaseshift) + My*cos(phaseshift);
    initM = M;
    ...
    woff(m) = woffarr(length(posarr)/2);                                                            % for checking
    ...
    woffarr2 = 2*pi*gamma*(posarr2*(refgrad-CASL.delg)) + 2*pi*(-CASL.delb0/1000) + wposdif;            % in rad/ms
    woff(m) = woffarr2(length(posarr)/2);                                                                              % for checking    
...
Panel(14,:) = {'PhaseInc (deg)',num2str(180*CASL.phaseinc/pi),'Output'};

Panel(15,:) = {'','','Output'};
Panel(16,:) = {'SimulationWid (m)',num2str(CASL.simulationwid),'Output'};
Panel(17,:) = {'TimeInSim (ms)',num2str(CASL.timeinsim),'Output'};
Panel(18,:) = {'TimeInSlab (ms)',num2str(CASL.timeinslab),'Output'};
