...
function [CASL,err] = pCASL_Tagging_v1a_Func(CASL,INPUT)
...
INPUT.T1 = NMRPrms.T1;
INPUT.T2 = NMRPrms.T2;
INPUT.time = linspace(0,timeseg,length(RF));
...
Mout = M;
for n = 1:segments
    pos(n) = -fulltestwid/2 + NMRPrms.vel*timeseg*(n-1)/1000;
    %woff(n) = -2*pi*pos(n)*G*gamma;                              % in rad/ms
    
    posarr = pos(n) + NMRPrms.vel*rftime/1000;
    woffarr = -2*pi*posarr*G*gamma;                       % in rad/ms
    woff(n) = woffarr(1);                                       % for checking
