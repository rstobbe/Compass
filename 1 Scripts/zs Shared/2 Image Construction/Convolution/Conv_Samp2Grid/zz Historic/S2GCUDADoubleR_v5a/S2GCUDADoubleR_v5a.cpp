///==========================================================
/// (v5a)
///     - Primary change in 'm' code (for parallel GPUs)
///		- This code essentially same as (v4g)
///             - devicenum dropped from input
///             - compiled with 7.5
///             - (doesn't seem to matter for parfor if 'status' left in
///==========================================================
...
#include "math.h"
#include "string.h"
#include "CUDA_GeneralD_v1b.h"
#include "CUDA_ConvSamp2GridSplitRD_v1d.h"
#include "CUDA_DeviceManage_v1b.h"
...
// Load / Initialize                    
//-------------------------------------
Mat3DInitDbl(HCrtDat,CrtDatSz,Error);
if (strcmp(Error,"no error") != 0) {
	plhs[2] = mxCreateString(Error); return;
	}
...
for (int n=0; n<(nchunks-1); n++){	
	...
	mxSetProperty(prhs[9],0,"String",mxCreateString(Status));
	mexEvalString("drawnow");
	ConvSamp2GridSplitRD(HSampDat,HCrtDat,HKx,HKy,HKz,HKern,CrtDatSz,chunklen,KernSz,iKern,chW,SampDatAdr,Error);
	if (strcmp(Error,"no error") != 0) {
		plhs[2] = mxCreateString(Error); return;
		}
	SampDatAdr += chunklen;
...
mxSetProperty(prhs[9],0,"String",mxCreateString(Status));
mexEvalString("drawnow");
ConvSamp2GridSplitRD(HSampDat,HCrtDat,HKx,HKy,HKz,HKern,CrtDatSz,chunkrem,KernSz,iKern,chW,SampDatAdr,Error);
if (strcmp(Error,"no error") != 0) {
	plhs[2] = mxCreateString(Error); return;
	}
...
// Return Data                    
//-------------------------------------
Mat3DReturnDbl(CrtDat,HCrtDat,CrtDatSz,Error);
if (strcmp(Error,"no error") != 0) {
	plhs[2] = mxCreateString(Error); return;
	}
