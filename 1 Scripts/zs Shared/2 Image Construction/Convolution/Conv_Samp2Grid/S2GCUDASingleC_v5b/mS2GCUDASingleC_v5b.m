...
function [CDat,err] = mS2GCUDASingleC_v5b(Ksz,Kx,Ky,Kz,KERN,SampDat,CONV,StatLev,CUDA)
...
if isreal(SampDat0)
    SampDat0 = complex(SampDat0,zeros(size(SampDat0),'single'));        % make sure complex
end
Kx0 = single(Kx);
Ky0 = single(Ky);
Kz0 = single(Kz);
...
%------------------------------------
tic
[CDat,Test,Error] = S2GCUDASingleC_v5b(SampDat,Kx,Ky,Kz,Kern,iKern,chW,Ksz,chunklen,Stathands);
toc
%Test
