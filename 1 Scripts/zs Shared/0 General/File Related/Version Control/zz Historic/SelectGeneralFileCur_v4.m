...
function [SCRPTipt,SCRPTGBL,err] = SelectGeneralFileCur_v4(SCRPTipt,SCRPTGBL)
...
elseif strcmp(RWSUIGBL.Key,'shift')
    Func = SCRPTGBL.CurrentScript.Func;
    curloc = SCRPTPATHS(SCRPTGBL.RWSUI.panelnum).outloc;
else
    Func = SCRPTGBL.CurrentScript.Func;
    Struct = SCRPTGBL.CurrentScript.Struct;
    curloc = Struct.(Func).curloc;
...
[file,path] = uigetfile('*.*','Select File',curloc);
if path == 0
    err.flag = 4;
...
Status('done','General File Loaded');
