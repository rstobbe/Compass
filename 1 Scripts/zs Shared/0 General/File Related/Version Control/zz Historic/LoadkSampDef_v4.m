...
function [SCRPTipt,SCRPTGBL,err] = LoadkSampDef_v4(SCRPTipt,SCRPTGBL)
...
switch defaultfunc
    case 'Make'
        ...
        SCRPTipt(SCRPTGBL.RWSUI.curpanipt).entrystruct.('LoadkSampCur_v4').curloc = path;
        Status('done','Default Directory Selected');

    case 'Load From'        
        [file,path] = uigetfile('KSMP*.mat','Select k-Space Sampling File',defloc);
        if path == 0
            err.flag = 4;
        ...
        SCRPTipt(SCRPTGBL.RWSUI.curpanipt).entrystruct.('LoadkSampCur_v4').curloc = loc;
