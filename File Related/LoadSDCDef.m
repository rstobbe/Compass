...
function [SCRPTipt,SCRPTGBL,err] = LoadSDCDef(SCRPTipt,SCRPTGBL)
...
if isfield(saveData0,'SDCS')
    %------------------------------------------
    % Show Info
    %------------------------------------------
    FIGOBJS.(SCRPTGBL.RWSUI.tab).Info.String = saveData0.SDCS.ExpDisp;
    SCRPTipt(SCRPTGBL.RWSUI.curpanipt).entrystruct.display = saveData0.SDCS.ExpDisp;
end

if not(isempty(saveData0)) && not(isfield(saveData0,'SDCS'))
